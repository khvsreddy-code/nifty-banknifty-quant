...
def trader_oi_battlefield(df, spot, step, levels):
    """Trader-first OI battlefield: compact, readable and centered on the nearest strikes."""
    x = strike_window(df, spot, step, 11).copy()
    if x.empty:
        st.info("Option-chain battlefield is unavailable.")
        return
    ...
    for _, r in x.iterrows():
        ...
        ce_doi = float(r.get("ce_doi_5m", 0) or 0)
        pe_doi = float(r.get("pe_doi_5m", 0) or 0)
        ce_state = str(r.get("ce_state", ""))
        pe_state = str(r.get("pe_state", ""))
        atm = abs(strike - spot) < step * 0.5
        eor = abs(strike - levels.get("eor", 10**9)) < step * 0.5
        eos = abs(strike - levels.get("eos", -10**9)) < step * 0.5

        def badge(oi, doi, state):
            if abs(doi) < 1:
                shift = "—"
            elif doi > 0:
                shift = "↑"
                ...
                shift = "↓"
            return f"{format_oi(oi)} {shift}"

        zone = "ATM" if atm else "EOR" if eor else "EOS" if eos else ""
        rows.append({
            "Strike": int(strike),
            "CE OI": badge(ce_oi, ce_doi, ce_state),
            "CE ΔOI": f"{ce_doi:+,.0f}",
            "Level": zone,
            "PE ΔOI": f"{pe_doi:+,.0f}",
            "PE OI": badge(pe_oi, pe_doi, pe_state),
        })
    out = pd.DataFrame(rows)
    st.dataframe(out, use_container_width=True, hide_index=True, height=390)
    st.caption("↑ = OI building • ↓ = OI reducing • ATM/EOS/EOR mark the decision area. Raw option Greeks and detailed state labels are in Quant Details.")
...
def render_index_v3(name, token):
    ...
    with w2:
        ...
        st.dataframe(pd.DataFrame(level_rows, columns=["Level", "Price", "Meaning"]),
