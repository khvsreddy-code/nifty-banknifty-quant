...
def decision_quality_score(score, scenario_spread):
    """Evidence strength only. This deliberately ignores data completeness."""
    q = 42 + abs(float(score)) * 0.38 + max(0.0, float(scenario_spread)) * 0.22
    return int(max(0, min(100, round(q))))


def data_quality_report(df, spot, expiry, market=None, futures=None, structure=None, vix=0.0, breadth_proxy=None):
    """Real data reliability score. Missing inputs earn zero; they are never treated as valid zeros.

    The score is intentionally conservative: 100 requires all critical inputs to be
    present and internally populated. Intraday metrics are time-aware so the first
    few minutes are not incorrectly labelled as bad data simply because the opening
    range/15-minute history has not matured yet.
    """
    market = market or {}
    futures = futures or {}
    ...
    def add(label, points, ok, status):
        details.append({"label": label, "points": points if ok else 0, "max": points, "ok": bool(ok), "status": status})

    add("Spot / LTP", 15, float(spot or 0) > 0, "live" if float(spot or 0) > 0 else "missing")

    chain_ok = not df.empty and len(df) >= 5
    add("Option chain", 20, chain_ok, f"{len(df)} strikes" if chain_ok else "missing")

    oi_ok = False
    if not df.empty:
        ce = pd.to_numeric(df.get("ce_oi", 0), errors="coerce").fillna(0)
        pe = pd.to_numeric(df.get("pe_oi", 0), errors="coerce").fillna(0)
        cdo = pd.to_numeric(df.get("ce_doi_5m", 0), errors="coerce").fillna(0)
        pdo = pd.to_numeric(df.get("pe_doi_5m", 0), errors="coerce").fillna(0)
        oi_ok = ((ce > 0) | (pe > 0)).sum() >= 5 and ((cdo != 0) | (pdo != 0)).sum() >= 1
    add("OI + ΔOI", 15, oi_ok, "populated" if oi_ok else "waiting / unavailable")

    bars = int(market.get("bars", 0) or 0)
    candle_ok = bars >= 5
    add("Intraday candles", 15, candle_ok, f"{bars} bars" if bars else "missing")

    vwap = float(market.get("vwap", 0) or 0)
    vwap_ok = vwap > 0 and bars >= 2
    add("VWAP", 10, vwap_ok, "calculated" if vwap_ok else "not ready")

    or_low, or_high = float(market.get("or_low", 0) or 0), float(market.get("or_high", 0) or 0)
    # The opening range is considered mature after 15 one-minute bars. Before that,
    # it is a normal 'building' state rather than an error, but it does not receive points.
    or_ok = bars >= 15 and or_low > 0 and or_high > 0 and or_high >= or_low
    add("Opening range", 10, or_ok, "ready" if or_ok else ("building" if bars > 0 else "missing"))

    futures_ok = bool(futures.get("available")) and np.isfinite(float(futures.get("basis", np.nan)))
    add("Futures / basis", 5, futures_ok, "live" if futures_ok else "unavailable")

    breadth_ok = breadth_proxy is not None and np.isfinite(float(breadth_proxy)) and abs(float(breadth_proxy)) <= 1.0
    add("Breadth / participation", 5, breadth_ok, "tracked" if breadth_ok else "not tracked")

    vix_ok = float(vix or 0) > 0
    add("India VIX", 5, vix_ok, "live" if vix_ok else "unavailable")

    score = int(round(sum(x["points"] for x in details)))
    if score >= 90:
        grade = "EXCELLENT"
    elif score >= 75:
        grade = "GOOD"
...
def signal_quality(score, data_quality, scenario_spread):
    """Backward-compatible effective confidence, capped by data reliability."""
    decision = decision_quality_score(score, scenario_spread)
    return int(min(decision, int(data_quality)))
...
def scenario_model_v3(df, spot, levels, step, pcr, dce, dpe, market, structure):
    ...
    # Scenario weights are deliberately not presented as calibrated win probabilities.
    # A neutral score should favour RANGE rather than manufacture a near-even 3-way split.
    up_raw = 30 + score*0.40
    down_raw = 30 - score*0.40
    range_raw = 40.0
    vals = np.array([max(5, up_raw), max(10, range_raw), max(5, down_raw)], dtype=float)
...
def display_metric_value(value, fmt="{:.0f}", placeholder="—"):
    try:
        x = float(value)
        return placeholder if not np.isfinite(x) or x == 0 else fmt.format(x)
    except Exception:
        return placeholder
...
def render_index_v3(name, token):
    ...
    breadth_proxy = st.session_state.get(f"breadth_proxy_{name}")
    ...
    breadth_proxy = st.session_state.get(f"breadth_proxy_{name}")
    dq, dq_grade, dq_details = data_quality_report(
        df, spot, expiry, market, futures, structure, vix, breadth_proxy
    )
    spread = max(scenario["up"], scenario["down"]) - scenario["range"]
    decision_quality = decision_quality_score(scenario["score"], spread)
    setup_confidence = min(decision_quality, dq)
    quality = setup_confidence  # compatibility for downstream panels/snapshots
    ...
    if setup_confidence < 45:
        state_icon, state_label, cls = "🟡", "WAIT — DATA INCOMPLETE", "range"
    elif primary == "UPSIDE":
    ...
    <div class="trader-shell">
      <div class="decision">
        <div class="box {cls}">
          ...
          <div class="sub">Model pressure • {abs(score):.0f}/100</div>
        </div>
        <div class="box">
        ...
        <div class="box">
          <div class="label">Data quality</div><div class="big">{dq}/100</div>
          <div class="sub">{dq_grade} • missing inputs earn 0</div>
        </div>
        <div class="box">
          <div class="label">Decision quality</div><div class="big">{decision_quality}/100</div>
          <div class="sub">Evidence strength only</div>
        </div>
        <div class="box">
          <div class="label">Setup confidence</div><div class="big">{setup_confidence}/100</div>
          <div class="sub">Capped by data quality</div>
        </div>
      </div>
    </div>
    <div class="pulse-card" style="margin-top:10px">
      <div class="pulse-top"><b>SCENARIO WEIGHT</b> • not a calibrated win probability</div>
      <div class="pulse-main">↑ {scenario['up']}% &nbsp;·&nbsp; ↔ {scenario['range']}% &nbsp;·&nbsp; ↓ {scenario['down']}%</div>
      <div class="pulse-sub">Evidence-first: {gate_status} • Effective setup confidence: {setup_confidence}/100</div>
    </div>
    """, unsafe_allow_html=True)

    vwap_text = display_metric_value(vwap, "{:,.0f}")
    or_low = display_metric_value(market.get("or_low", 0), "{:,.0f}")
    or_high = display_metric_value(market.get("or_high", 0), "{:,.0f}")
    sh = display_metric_value(market.get("session_high", 0), "{:,.0f}")
    sl = display_metric_value(market.get("session_low", 0), "{:,.0f}")
    basis = futures.get("basis")
    basis_text = display_metric_value(basis, "{:+.1f}")
    pcr_text = f"{pcr:.2f}" if np.isfinite(pcr) else "—"
    vix_text = f"{vix:.2f}" if vix else "—"
    r5 = market.get("ret5")
    r15 = market.get("ret15")
    r5_text = f"{float(r5):+.2f}%" if r5 is not None and int(market.get("bars",0) or 0) >= 6 else "—"
    r15_text = f"{float(r15):+.2f}%" if r15 is not None and int(market.get("bars",0) or 0) >= 16 else "—"
    st.markdown(f"""
    <div class="mini-grid">
      <div class="mini"><div class="k">VWAP</div><div class="v">{vwap_text}{(' ↑' if spot>vwap else ' ↓') if vwap else ''}</div></div>
      <div class="mini"><div class="k">Opening range</div><div class="v">{or_low}–{or_high}</div></div>
      <div class="mini"><div class="k">Session H/L</div><div class="v">{sl}–{sh}</div></div>
      <div class="mini"><div class="k">Futures basis</div><div class="v">{basis_text}</div></div>
      <div class="mini"><div class="k">PCR</div><div class="v">{pcr_text}</div></div>
      <div class="mini"><div class="k">India VIX</div><div class="v">{vix_text}</div></div>
      <div class="mini"><div class="k">5m</div><div class="v">{r5_text}</div></div>
      <div class="mini"><div class="k">15m</div><div class="v">{r15_text}</div></div>
    </div>
    """, unsafe_allow_html=True)

    # Transparent data-status row: tells the trader why the quality score is what it is.
    status_items = []
    for item in dq_details:
        icon = "✓" if item["ok"] else "⚠"
        status_items.append(f"<span class='dq-item {"ok" if item["ok"] else "warn"}'>{icon} {item['label']}: {item['status']}</span>")
    st.markdown("<div class='dq-strip'><div class='dq-title'>DATA STATUS</div>" + "".join(status_items) + "</div>", unsafe_allow_html=True)
...
def inject_trader_css():
    ...
    .decision{display:grid;grid-template-columns:1.25fr .8fr .8fr .8fr .8fr;gap:10px}
    ...
    .change-icon{font-size:19px}.proximity-card{background:#fff;border:1px solid #e5eaf0;border-radius:15px;padding:12px 14px;box-shadow:0 5px 18px rgba(15,23,42,.045)}
    ...
    .dq-strip{display:flex;align-items:center;gap:7px;flex-wrap:wrap;background:#fff;border:1px solid #e5eaf0;border-radius:12px;padding:9px 12px;margin:8px 0 14px;color:#475569;font-size:11px}.dq-title{font-size:10px;font-weight:900;letter-spacing:.07em;color:#172033;margin-right:3px}.dq-item{padding:4px 7px;border-radius:999px;background:#f8fafc;border:1px solid #e7ebf0}.dq-item.ok{color:#047857;background:#ecfdf5;border-color:#bbf7d0}.dq-item.warn{color:#a16207;background:#fffbeb;border-color:#fde68a}.pulse-top{font-size:12px;color:#475569}.pulse-main{font-size:15px;font-weight:800;margin-top:6px}.pulse-sub{font-size:11px;color:#64748b;margin-top:3px}
