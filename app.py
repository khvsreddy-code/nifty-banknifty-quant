...
def render_upstox_tv_chart(candles, spot, levels, name, tf_minutes=5, token=''):
...
const chart=LightweightCharts.createChart(mainEl,{width:mainEl.clientWidth,height:480,layout:{background:{type:'solid',color:'#ffffff'},textColor:'#334155',fontFamily:'Inter,system-ui,sans-serif',fontSize:12},grid:{vertLines:{color:'#edf1f5'},horzLines:{color:'#edf1f5'}},crosshair:{mode:LightweightCharts.CrosshairMode.Normal,vertLine:{color:'#94a3b8',width:1,style:LightweightCharts.LineStyle.Dashed,labelBackgroundColor:'#334155'},horzLine:{color:'#94a3b8',width:1,style:LightweightCharts.LineStyle.Dashed,labelBackgroundColor:'#334155'}},rightPriceScale:{borderColor:'#d8e0e8',scaleMargins:{top:.07,bottom:.08},autoScale:true},timeScale:{borderColor:'#d8e0e8',timeVisible:true,secondsVisible:false,rightOffset:5,barSpacing:7,minBarSpacing:2},handleScroll:{mouseWheel:true,pressedMouseMove:true,horzTouchDrag:true,vertTouchDrag:true},handleScale:{axisPressedMouseMove:{time:true,price:true},mouseWheel:true,pinch:true},localization:{priceFormatter:p=>Number(p).toLocaleString('en-IN',{maximumFractionDigits:2})}});
const cs=chart.addSeries(LightweightCharts.CandlestickSeries,{upColor:'#22c55e',downColor:'#ef4444',borderUpColor:'#22c55e',borderDownColor:'#ef4444',wickUpColor:'#22c55e',wickDownColor:'#ef4444'});cs.setData(DATA);const ls=chart.addSeries(LightweightCharts.LineSeries,{color:'#d8e2ee',lineWidth:2,visible:false,priceLineVisible:false,lastValueVisible:false});ls.setData(DATA.map(x=>({time:x.time,value:x.close})));
...
// Browser-side live bridge. The iframe owns the price loop.
 ...
 async function startRealtime(){
   const SB_URL=__SB_URL__,SB_KEY=__SB_KEY__;
   if(!SB_URL||!SB_KEY){await supabaseLatest();return;}
   try{
...
addLine('ema20',ema(20),'#60a5fa');setPane('RSI');fit();renderDrawings();new ResizeObserver(()=>{chart.applyOptions({width:mainEl.clientWidth});pane.applyOptions({width:paneEl.clientWidth});volChart.applyOptions({width:volEl.clientWidth});renderDrawings()}).observe(mainEl);chart.timeScale().subscribeVisibleTimeRangeChange(()=>{renderDrawings();try{let r=chart.timeScale().getVisibleRange();if(r){volChart.timeScale().setVisibleRange(r);pane.timeScale().setVisibleRange(r)}}catch(e){}});
    ...
    html=html.replace('__DATA__',payload).replace('__LEVELS__',lvl).replace('__TF_MINUTES__',tf).replace('__NAME__',str(name).replace('&','&amp;').replace('<','&lt;').replace('>','&gt;')).replace('__NAME_JSON__',nm).replace('__SB_URL__',sb_url_json).replace('__SB_KEY__',sb_key_json).replace('__LIVE_SYMBOL__',_json.dumps(str(name)))
