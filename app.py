...
def oi_heatmap(df, spot, step, levels=None):
    """Real visual OI heatmap rendered with HTML/CSS; no matplotlib dependency."""
    levels = levels or {}
    ...
    ce_max = max(float(pd.to_numeric(x["ce_oi"], errors="coerce").fillna(0).max()), 1.0)
    pe_max = max(float(pd.to_numeric(x["pe_oi"], errors="coerce").fillna(0).max()), 1.0)

    def bg(v, vmax, side):
        a = .08 + min(1, max(0, float(v)/vmax))*.68
        return f"rgba(220,38,38,{a:.2f})" if side == "ce" else f"rgba(5,150,105,{a:.2f})"
    def shift(v):
        v = float(v or 0)
        if abs(v) < 1: return "—", "oi-flat"
        return (("↑ " if v > 0 else "↓ ") + format_oi(abs(v))), ("oi-up" if v > 0 else "oi-down")

    rows = []
    for _, r in x.iterrows():
        strike=float(r["strike"]); ce=float(r.get("ce_oi",0) or 0); pe=float(r.get("pe_oi",0) or 0)
        cd=float(r.get("ce_doi_5m",0) or 0); pd_=float(r.get("pe_doi_5m",0) or 0)
        if abs(strike-spot) < step*.5: tag=("ATM","oi-atm")
        elif abs(strike-levels.get("eor",1e12)) < step*.5: tag=("EOR","oi-eor")
        elif abs(strike-levels.get("eos",-1e12)) < step*.5: tag=("EOS","oi-eos")
        else: tag=("","")
        ctext,ccls=shift(cd); ptext,pcls=shift(pd_)
        taghtml=f'<span class="oi-tag {tag[1]}">{tag[0]}</span>' if tag[0] else ""
        rows.append(f"""
        <div class="oi-row">
          <div class="oi-cell ce"><div class="oi-fill" style="background:{bg(ce,ce_max,'ce')}">
            <div class="oi-main">{format_oi(ce)}</div><div class="oi-shift {ccls}">{ctext} ΔOI</div>
          </div></div>
          <div class="oi-strike"><strong>{int(strike):,}</strong>{taghtml}</div>
          <div class="oi-cell pe"><div class="oi-fill" style="background:{bg(pe,pe_max,'pe')}">
            <div class="oi-main">{format_oi(pe)}</div><div class="oi-shift {pcls}">{ptext} ΔOI</div>
          </div></div>
        </div>""")
    html = f"""
    <div class="oi-wrap">
      <div class="oi-head"><div class="ce">CE OI / 5m ΔOI</div><div class="mid">STRIKE</div><div class="pe">PE OI / 5m ΔOI</div></div>
      {''.join(rows)}
      <div class="oi-legend">
        <span>🔴 <b>CE</b> = resistance pressure</span><span>🟢 <b>PE</b> = support pressure</span>
        <span>↑ building</span><span>↓ unwinding</span><span>ATM / EOS / EOR = decision zones</span>
      </div>
    </div>"""
...
def trader_oi_battlefield(df, spot, step, levels):
    ...
    if x.empty:
        ...
        return
    st.markdown("### OI pressure map")
    st.caption("Darker cells = larger OI walls. Arrows show the latest 5-minute OI shift.")
    oi_heatmap(df, spot, step, levels)
    x["strike"]=pd.to_numeric(x["strike"],errors="coerce")
    x=x.dropna(subset=["strike"])
    above=x[x["strike"]>=spot].sort_values("ce_oi",ascending=False).head(3)
    below=x[x["strike"]<=spot].sort_values("pe_oi",ascending=False).head(3)
    c1,c2=st.columns(2)
    with c1:
        st.markdown("**🔴 Strongest CE walls**")
        for _,r in above.iterrows():
            st.write(f"**{int(r.strike):,}** · {format_oi(r.ce_oi)} OI · ΔOI {float(r.ce_doi_5m):+,.0f}")
    with c2:
        st.markdown("**🟢 Strongest PE walls**")
        for _,r in below.iterrows():
            st.write(f"**{int(r.strike):,}** · {format_oi(r.pe_oi)} OI · ΔOI {float(r.pe_doi_5m):+,.0f}")
...
def render_index_v3(name, token):
    ...
    if primary == "UPSIDE":
        state_icon, state_label = "🟢", "BULLISH"
    elif primary == "DOWNSIDE":
        state_icon, state_label = "🔴", "BEARISH"
    else:
        state_icon, state_label = "🟡", "RANGE / WAIT"

    st.markdown(f"# {name}  <span style='font-size:0.55em;color:#777'>Intraday OI Terminal</span>", unsafe_allow_html=True)
    st.caption(f"{expiry} expiry • {phase} • Data quality {dq}/100 • Evidence-first policy: {gate_status}")

    hero = st.container(border=True)
    with hero:
        h1, h2, h3 = st.columns([1.35, 1.0, 1.0])
        with h1:
            st.markdown(f"### {state_icon} {state_label}")
            st.metric("SPOT", f"{spot:,.2f}", f"{change_pct:+.2f}%")
        with h2:
            st.markdown("### Current bias")
            st.metric("MODEL BIAS", f"{abs(score):.0f}/100", primary)
            st.caption(f"Scenario weights: ↑ {scenario['up']}% • ↔ {scenario['range']}% • ↓ {scenario['down']}%")
        with h3:
            st.markdown("### Market confirmation")
            confirmations = []
            if vwap and ((spot > vwap and score > 0) or (spot < vwap and score < 0)):
                confirmations.append("VWAP")
            if futures.get("available"):
                fb = futures.get("basis", 0)
                if (fb >= 0 and score >= 0) or (fb <= 0 and score <= 0):
                    confirmations.append("Futures")
            if abs(dce - dpe) > 0:
                confirmations.append("OI")
            st.metric("QUALITY", f"{quality}/100")
            st.caption(" • ".join(confirmations) if confirmations else "Mixed / waiting for confirmation")
    ...
    with w2:
        ...
        level_rows = [
            ("EOR + 1", levels["eor1"], "🔴 extension"),
            ("EOR", levels["eor"], f"🔴 resistance • {levels['resistance_score']:.0f}/100"),
            ("SPOT", spot, "● current"),
            ("EOS", levels["eos"], f"🟢 support • {levels['support_score']:.0f}/100"),
            ("EOS - 1", levels["eos1"], "🟢 extension"),
        ]
        cards = []
        for label, price, meaning in level_rows:
            cls = "eor" if "EOR" in label else "eos" if "EOS" in label else "atm" if label == "SPOT" else ""
            cards.append(f'<div class="tp-level {cls}"><div class="lab">{label}</div><div class="num">{price:,.0f}</div><div class="tp-sub">{meaning}</div></div>')
        st.markdown('<div class="tp-levels">' + ''.join(cards) + '</div>', unsafe_allow_html=True)

    # -----------------------------
    # NEXT MOVE CARD
    # -----------------------------
    st.markdown("## 🔮 Next move — what needs to happen?")
    ...
    pulse_df = pd.DataFrame(sorted(movers, reverse=True)[:8],
                            columns=["Magnitude","Side","Strike","ΔOI","State","Change"])
    if not pulse_df.empty:
        pulse_df["Reading"] = pulse_df.apply(
            lambda r: ("🟢 " if r["Side"] == "PE" and r["ΔOI"] > 0 else "🔴 " if r["Side"] == "CE" and r["ΔOI"] > 0 else "⚪ ") +
                      f"{r['Side']} {int(r['Strike']):,}: {r['Change']}", axis=1)
        st.dataframe(pulse_df[["Reading","ΔOI","State"]], use_container_width=True, hide_index=True, height=285)
    else:
    ...
    # MOVERS + RESEARCH
    # -----------------------------
    with st.expander(f"🚦 What is moving {name}?"):
...
def inject_trader_ui():
    st.markdown("""
    <style>
    .stApp{background:#f4f6fa;color:#172033}.block-container{max-width:1500px;padding:1.1rem 2rem 3rem}
    [data-testid="stMetric"]{background:#fff;border:1px solid #e4e8ef;border-radius:14px;padding:.72rem .9rem;box-shadow:0 3px 14px rgba(15,23,42,.045)}
    .oi-wrap{background:#fff;border:1px solid #dfe5ec;border-radius:16px;overflow:hidden;box-shadow:0 5px 22px rgba(15,23,42,.055)}
    .oi-head{display:grid;grid-template-columns:1fr 120px 1fr;background:#f8fafc;border-bottom:1px solid #e5e9ef}
    .oi-head div{padding:12px 16px;font-size:11px;font-weight:850;letter-spacing:.07em;text-transform:uppercase}
    .oi-head .ce{text-align:right;color:#b42318}.oi-head .mid{text-align:center;color:#475569}.oi-head .pe{color:#047857}
    .oi-row{display:grid;grid-template-columns:1fr 120px 1fr;min-height:58px;border-bottom:1px solid #edf0f4}.oi-row:last-child{border-bottom:0}
    .oi-cell{display:flex;align-items:center;gap:9px;padding:7px 14px}.oi-cell.ce{justify-content:flex-end;text-align:right}.oi-cell.pe{justify-content:flex-start;text-align:left}
    .oi-fill{border-radius:8px;padding:9px 11px;min-width:120px}.oi-main{font-size:15px;font-weight:850;color:#172033}.oi-shift{font-size:11px;font-weight:750;margin-top:3px}
    .oi-up{color:#15803d}.oi-down{color:#b91c1c}.oi-flat{color:#64748b}
    .oi-strike{display:flex;flex-direction:column;align-items:center;justify-content:center;border-left:1px solid #edf0f4;border-right:1px solid #edf0f4;background:#fff}
    .oi-strike strong{font-size:15px}.oi-tag{margin-top:4px;padding:2px 7px;border-radius:999px;font-size:9px;font-weight:850;letter-spacing:.05em}
    .oi-atm{background:#e2e8f0;color:#334155}.oi-eor{background:#fee2e2;color:#b91c1c}.oi-eos{background:#dcfce7;color:#166534}
    .oi-legend{padding:11px 14px;background:#fbfcfe;color:#64748b;font-size:11px;display:flex;gap:18px;flex-wrap:wrap}
    @media(max-width:900px){.block-container{padding:1rem}.oi-head,.oi-row{grid-template-columns:1fr 88px 1fr}.oi-fill{min-width:75px}}
    @media(max-width:600px){.block-container{padding:.65rem}.oi-fill{min-width:60px;padding:7px}.oi-main{font-size:13px}}
    </style>
...
def main():
    ...
    with st.sidebar:
        ...
        st.caption(f"Live refresh: every {refresh}s • {datetime.now(IST).strftime('%H:%M:%S IST')}")
    ...
    st.caption("NIFTY & BANK NIFTY • live OI battlefield • EOS/EOR levels • futures confirmation • confluence • absorption • regime • research gate")
