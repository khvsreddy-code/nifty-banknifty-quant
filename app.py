...
def render_market_explorer(token):
    st.markdown("## 📈 Markets")
    st.caption("Stocks, NSE/BSE futures, NSE/BSE options and MCX contracts use the same true browser-live chart. Streamlit does not poll or rebuild the chart.")

    tabs=st.tabs(["Stocks","Futures","Options","MCX"])
    with tabs[0]:
        q=st.text_input("Stock symbol", value="RELIANCE", key="market_stock_query",
                        help="Examples: RELIANCE, HDFCBANK, INFY, TCS")
        rows=search_market_instruments(token,q,"NSE,BSE","EQ","EQ")
        if rows:
            labels=[_market_result_label(r) for r in rows]
            idx=st.selectbox("Stock",range(len(rows)),format_func=lambda i:labels[i],key="market_stock_pick")
            render_generic_market_chart(token,rows[idx],5)
        else:
            st.info("No equity matched that search.")

    with tabs[1]:
        q=st.text_input("Future search", value="NIFTY", key="market_future_query",
                        help="Examples: NIFTY, BANKNIFTY, RELIANCE, CRUDEOIL")
        rows=search_market_instruments(token,q,"NSE,BSE","FO","FUT","current_month")
        if rows:
            labels=[_market_result_label(r) for r in rows]
            idx=st.selectbox("Future contract",range(len(rows)),format_func=lambda i:labels[i],key="market_future_pick")
            render_generic_market_chart(token,rows[idx],5)
        else:
            st.info("No current-month NSE/BSE future matched that search.")

    with tabs[2]:
        q=st.text_input("Option search", value="NIFTY", key="market_option_query",
                        help="Examples: NIFTY, BANKNIFTY, RELIANCE, HDFCBANK")
        rows=search_market_instruments(token,q,"NSE,BSE","FO","CE,PE","current_month")
        if rows:
            # Prefer ATM-ish results when Upstox returns many contracts.
            labels=[_market_result_label(r) for r in rows]
            idx=st.selectbox("Option contract",range(len(rows)),format_func=lambda i:labels[i],key="market_option_pick")
            render_generic_market_chart(token,rows[idx],5)
        else:
            st.info("No current-month option matched that search.")

    with tabs[3]:
        q=st.text_input("MCX contract search", value="CRUDEOIL", key="market_mcx_query",
                        help="Examples: CRUDEOIL, GOLD, SILVER, NATURALGAS")
        rows=search_market_instruments(token,q,"MCX","FO","FUT","current_month")
        if rows:
            labels=[_market_result_label(r) for r in rows]
            idx=st.selectbox("MCX future",range(len(rows)),format_func=lambda i:labels[i],key="market_mcx_pick")
