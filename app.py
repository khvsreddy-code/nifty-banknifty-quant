...
st.set_page_config(page_title="TradingCoreOS", page_icon="⚡", layout="wide", initial_sidebar_state="expanded")
st.markdown("""
<style>
...
def _supabase_config():
    ...
    Set SUPABASE_SERVICE_ROLE_KEY in Streamlit Secrets for reliable recorder
    writes. It is never passed to the chart iframe.
    """
    try:
        url = str(st.secrets.get("SUPABASE_URL", "")).strip().rstrip("/")
        key = str(st.secrets.get("SUPABASE_SERVICE_ROLE_KEY", st.secrets.get("SUPABASE_KEY", ""))).strip()
    except Exception:
        return "", ""
    return url, key

def _supabase_browser_config():
    """Return only the publishable key that may safely reach the browser."""
    try:
        url = str(st.secrets.get("SUPABASE_URL", "")).strip().rstrip("/")
...
def _json_safe(value):
    """Normalize pandas/numpy values before sending JSON to PostgREST."""
...
def _post_supabase(table, rows, on_conflict=None):
    ...
    try:
        r = requests.post(endpoint, params=params, headers=headers, json=_json_safe(rows),
...
def _get_supabase(table, params=None):
    """Server-side read used by the research recorder only."""
    url, _ = _supabase_config()
    headers = _supabase_headers()
    if not url or not headers:
        return []
    try:
        r = requests.get(f"{url}/rest/v1/{table}", params=params or {}, headers=headers,
                         timeout=SUPABASE_TIMEOUT)
...
def save_supabase_minute(name, spot, expiry, df, levels, scenario, market,
                         structure, futures, regime, events, market_state=None):
    ...
    # One complete record per index/minute is much more resilient than a
    # chain of dependent REST inserts. The JSON payload keeps all raw fields
    # for research while a single upsert makes recorder success atomic.
    snapshot_row = {
        "captured_at": ts,
        "capture_minute": ts,
        "trading_date": today,
    ...
    saved, _ = _post_supabase(
        "tradingcore_snapshots", [snapshot_row], "index_name,capture_minute"
    )
    okm = oko = okl = okp = saved
    nopt = len(option_rows) if saved else 0
...
# ---------------- PHASE 1A: PREDICTION OUTCOME RECORDER ----------------
# These rows are intentionally observations, not a trading recommendation and
# not a claim that the current evidence score is a calibrated probability.
OUTCOME_HORIZONS = (5, 15, 30, 60)

def _event_time(value):
    try:
        return pd.to_datetime(value, utc=True).to_pydatetime().astimezone(IST)
    except Exception:
        return None

def _prediction_fingerprint(scenario, levels, regime, phase, spot):
    """Record a new event only when its decision context meaningfully changes."""
    score = float(scenario.get("score", 0) or 0)
    support = float(levels.get("support_precision") or levels.get("support") or spot)
    resistance = float(levels.get("resistance_precision") or levels.get("resistance") or spot)
    return "|".join((
        str(scenario.get("primary", "RANGE / WAIT")),
        str(regime.get("regime", "UNKNOWN")), str(phase),
        str(int(round(score / 10.0) * 10)),
        str(round(support, 1)), str(round(resistance, 1)),
    ))

def record_prediction_event(name, spot, expiry, levels, scenario, market, structure,
                            futures, regime, phase, events, state_vector):
    """Save an immutable prediction context when its state changes.

    A single persistent ID links the original observation to all later
    5/15/30/60-minute outcomes. A state fingerprint suppresses noisy duplicate
    records while retaining reversals, regime changes and level changes.
    """
    if not supabase_enabled():
        return {"enabled": False, "recorded": False}
    now = _supabase_minute()
    fingerprint = _prediction_fingerprint(scenario, levels, regime, phase, spot)
    cache_key = f"_prediction_fp_{name}"
    if st.session_state.get(cache_key) == fingerprint:
        return {"enabled": True, "recorded": False, "reason": "unchanged"}
    latest = _get_supabase("prediction_events", {
        "select": "state_fingerprint", "index_name": f"eq.{name}",
        "order": "entry_time.desc", "limit": "1",
    })
    if latest and latest[0].get("state_fingerprint") == fingerprint:
        st.session_state[cache_key] = fingerprint
        return {"enabled": True, "recorded": False, "reason": "unchanged"}
    direction = str(scenario.get("primary", "RANGE / WAIT"))
    event_seed = f"{name}|{now.isoformat()}|{fingerprint}"
    event_id = hashlib.sha256(event_seed.encode("utf-8")).hexdigest()[:32]
    upside = direction == "UPSIDE"
    row = {
        "event_id": event_id, "entry_time": now.isoformat(),
        "trading_date": now.date().isoformat(), "index_name": name,
        "expiry": str(expiry) if expiry else None, "entry_price": _json_number(spot),
        "direction": direction, "evidence_score": _json_number(scenario.get("score")),
        "target_1": _json_number(levels.get("eor1") if upside else levels.get("eos1")),
        "target_2": _json_number(levels.get("eor2") if upside else levels.get("eos2")),
        "stop_level": _json_number(levels.get("eos") if upside else levels.get("eor")),
        "regime": str(regime.get("regime", "UNKNOWN")), "phase": str(phase),
        "state_fingerprint": fingerprint,
        "feature_snapshot": {
            "scenario": {k: scenario.get(k) for k in ("score", "primary", "up", "range", "down", "reasons")},
            "levels": levels, "market": market, "option_structure": structure,
            "futures": futures, "events": events, "market_state": state_vector,
            "breadth_context": st.session_state.get(f"breadth_context_{name}", {}),
        },
    }
    ok, _ = _post_supabase("prediction_events", [row], "event_id")
    if ok:
        st.session_state[cache_key] = fingerprint
    return {"enabled": True, "recorded": bool(ok), "event_id": event_id}

def settle_prediction_outcomes(name):
    """Settle mature events from recorded one-minute spot observations.

    This is safe to call on every Streamlit rerun: the database unique key
    makes each event/horizon outcome idempotent. MFE/MAE are explicitly based
    on captured minute prices, so they never claim tick-level precision.
    """
    if not supabase_enabled():
        return {"enabled": False, "settled": 0, "pending": 0}
    today = datetime.now(IST).date().isoformat()
    events = _get_supabase("prediction_events", {
        "select": "event_id,entry_time,entry_price,direction,target_1,target_2,stop_level",
        "index_name": f"eq.{name}", "trading_date": f"eq.{today}",
        "order": "entry_time.asc", "limit": "250",
    })
    snapshots = _get_supabase("tradingcore_snapshots", {
        "select": "capture_minute,spot", "index_name": f"eq.{name}",
        "trading_date": f"eq.{today}", "order": "capture_minute.asc", "limit": "500",
    })
    prices = [(t, _json_number(r.get("spot"))) for r in snapshots
              if (t := _event_time(r.get("capture_minute"))) is not None and _json_number(r.get("spot")) is not None]
    now = datetime.now(IST)
    rows, pending = [], 0
    for event in events:
        entry_time = _event_time(event.get("entry_time")); entry = _json_number(event.get("entry_price"))
        if entry_time is None or entry is None or entry <= 0:
            continue
        path = [(t, p) for t, p in prices if t >= entry_time]
        for horizon in OUTCOME_HORIZONS:
            target_time = entry_time + timedelta(minutes=horizon)
            if now < target_time:
                pending += 1; continue
            candidates = [(t, p) for t, p in path if t >= target_time]
            if not candidates:
                pending += 1; continue
            settled_at, exit_price = candidates[0]
            if (settled_at - target_time).total_seconds() > 180:
                pending += 1; continue
            observed = [p for t, p in path if t <= settled_at]
            direction = event.get("direction")
            sign = 1 if direction == "UPSIDE" else -1 if direction == "DOWNSIDE" else 0
            raw_return = (exit_price / entry - 1.0) * 100.0
            directional_return = raw_return * sign if sign else None
            favorable_price = max(observed) if sign > 0 else min(observed)
            adverse_price = min(observed) if sign > 0 else max(observed)
            mfe = ((favorable_price / entry - 1.0) * 100.0 * sign) if sign else None
            mae = ((adverse_price / entry - 1.0) * 100.0 * sign) if sign else None
            target_1, target_2, stop = (_json_number(event.get(k)) for k in ("target_1", "target_2", "stop_level"))
            if sign > 0:
                target_1_hit = target_1 is not None and max(observed) >= target_1
                target_2_hit = target_2 is not None and max(observed) >= target_2
                stop_hit = stop is not None and min(observed) <= stop
            elif sign < 0:
                target_1_hit = target_1 is not None and min(observed) <= target_1
                target_2_hit = target_2 is not None and min(observed) <= target_2
                stop_hit = stop is not None and max(observed) >= stop
            else:
                target_1_hit = target_2_hit = stop_hit = None
            rows.append({
                "event_id": event["event_id"], "horizon_minutes": horizon,
                "settled_at": settled_at.isoformat(), "exit_price": exit_price,
                "raw_return_pct": raw_return, "directional_return_pct": directional_return,
                "mfe_pct_close_sampled": mfe, "mae_pct_close_sampled": mae,
                ...
                "stop_hit": stop_hit, "observation_count": len(observed),
            })
    ok, count = _post_supabase("prediction_outcomes", rows, "event_id,horizon_minutes")
    return {"enabled": True, "settled": count if ok else 0, "pending": pending, "events": len(events)}

def render_outcome_recorder_status(event_status, outcome_status):
    if not supabase_enabled():
        return
    detail = f"{outcome_status.get('settled', 0)} settled • {outcome_status.get('pending', 0)} pending"
    if event_status.get("recorded"):
        st.caption(f"🧪 Phase 1A evidence recorder • new state captured • {detail}")
    else:
        st.caption(f"🧪 Phase 1A evidence recorder • {detail}")

# ---------------- PHASE 1B: EVIDENCE REVIEW / CALIBRATION ----------------
def _wilson_interval(successes, total, z=1.96):
    """A conservative 95% interval for observed directional hit rate."""
    if total <= 0:
        return None, None
    p = successes / total
    denom = 1 + z * z / total
...
def phase_1b_evidence_frame(index_name):
    """Join immutable Phase 1A event context to its settled outcome labels."""
    if not supabase_enabled():
        return pd.DataFrame(), "Database recorder is not configured."
    rows = _get_supabase("prediction_outcomes", {
        "select": (
            "event_id,horizon_minutes,raw_return_pct,directional_return_pct,"
            "mfe_pct_close_sampled,mae_pct_close_sampled,direction_correct,"
            "target_1_hit,target_2_hit,stop_hit,observation_count,"
            "prediction_events!inner(index_name,direction,evidence_score,regime,phase,entry_time)"
        ),
        "prediction_events.index_name": f"eq.{index_name}",
        "order": "settled_at.desc", "limit": "2000",
    })
    flat = []
    for row in rows:
        event = row.get("prediction_events") or {}
        if event.get("direction") not in ("UPSIDE", "DOWNSIDE"):
            continue  # RANGE / WAIT is recorded, but is not a directional hit-rate claim.
        score = _json_number(event.get("evidence_score"))
        if score is None:
            continue
        magnitude = abs(score)
        bucket = "0–24" if magnitude < 25 else "25–49" if magnitude < 50 else "50–74" if magnitude < 75 else "75–100"
        flat.append({
            "Event": row.get("event_id"), "Horizon": int(row.get("horizon_minutes") or 0),
            "Direction": event.get("direction"), "Evidence bucket": bucket,
            "Regime": event.get("regime", "UNKNOWN"), "Phase": event.get("phase", "UNKNOWN"),
            "Raw return": _json_number(row.get("raw_return_pct")),
            "Directional return": _json_number(row.get("directional_return_pct")),
            "MFE (close sampled)": _json_number(row.get("mfe_pct_close_sampled")),
            "MAE (close sampled)": _json_number(row.get("mae_pct_close_sampled")),
            "Correct": row.get("direction_correct"), "Target 1": row.get("target_1_hit"),
            "Stop": row.get("stop_hit"), "Entry time": event.get("entry_time"),
        })
    return pd.DataFrame(flat), None

def render_phase_1b_evidence(index_name):
    """Display measured results; never relabel an evidence score as probability."""
    st.markdown("## 📐 Phase 1B — Evidence Review")
    frame, error = phase_1b_evidence_frame(index_name)
    if error:
        st.info(error)
        return
    if frame.empty:
        st.info("No settled directional Phase 1A outcomes yet. The recorder will populate this after the first 5/15/30/60-minute horizons mature.")
        return
    n_events = frame["Event"].nunique()
    n_labels = len(frame)
    c1, c2, c3 = st.columns(3)
    c1.metric("Independent events", n_events)
    c2.metric("Settled labels", n_labels)
    c3.metric("Evidence state", "READY FOR REVIEW" if n_events >= 100 else "COLLECTING EVIDENCE")
    st.caption("Current scenario scores remain evidence scores—not probabilities. Formal probability calibration is deferred until enough independent events exist across regimes.")

    summary_rows = []
    for horizon in OUTCOME_HORIZONS:
        d = frame[frame["Horizon"] == horizon].copy()
        d = d[d["Correct"].notna()]
        if d.empty:
            continue
        total = len(d); wins = int(d["Correct"].astype(bool).sum())
        lo, hi = _wilson_interval(wins, total)
        summary_rows.append({
            "Horizon": f"{horizon}m", "Samples": total,
            "Observed hit rate": wins / total, "95% interval": f"{lo:.1%} – {hi:.1%}",
            "Mean directional return": d["Directional return"].mean(),
            "Mean MFE": d["MFE (close sampled)"].mean(),
            "Mean MAE": d["MAE (close sampled)"].mean(),
            "Target 1 hit": d["Target 1"].dropna().astype(bool).mean() if d["Target 1"].notna().any() else np.nan,
            "Stop hit": d["Stop"].dropna().astype(bool).mean() if d["Stop"].notna().any() else np.nan,
        })
    if summary_rows:
        st.markdown("### Outcome quality by horizon")
        st.dataframe(pd.DataFrame(summary_rows).style.format({
            "Observed hit rate": "{:.1%}", "Mean directional return": "{:+.3f}%",
            "Mean MFE": "{:+.3f}%", "Mean MAE": "{:+.3f}%",
            "Target 1 hit": "{:.1%}", "Stop hit": "{:.1%}",
        }, na_rep="—"), use_container_width=True, hide_index=True)

    selected_horizon = st.selectbox("Evidence horizon", list(OUTCOME_HORIZONS), index=1,
                                    key=f"phase_1b_horizon_{index_name}")
    d = frame[(frame["Horizon"] == selected_horizon) & frame["Correct"].notna()].copy()
    if d.empty:
        return
    bucket_order = ["0–24", "25–49", "50–74", "75–100"]
    by_bucket = d.groupby("Evidence bucket", sort=False).agg(
        Samples=("Correct", "size"), Observed_hit_rate=("Correct", lambda s: s.astype(bool).mean()),
        Mean_directional_return=("Directional return", "mean"),
    ).reindex(bucket_order).dropna(subset=["Samples"]).reset_index()
    st.markdown(f"### Evidence-score reliability · {selected_horizon}m")
    st.dataframe(by_bucket.style.format({"Observed_hit_rate": "{:.1%}", "Mean_directional_return": "{:+.3f}%"}),
                 use_container_width=True, hide_index=True)
    st.caption("Do not compare buckets until each has at least 30 independent events; do not call any bucket a calibrated probability until validation across separate market days and regimes is complete.")

    by_regime = d.groupby("Regime").agg(
        Samples=("Correct", "size"), Observed_hit_rate=("Correct", lambda s: s.astype(bool).mean()),
        Mean_directional_return=("Directional return", "mean"),
    ).reset_index().sort_values("Samples", ascending=False)
    st.markdown("### Stability by market regime")
    st.dataframe(by_regime.style.format({"Observed_hit_rate": "{:.1%}", "Mean_directional_return": "{:+.3f}%"}),
                 use_container_width=True, hide_index=True)


def render_phase_6_model_readiness(index_name):
    """Prevent premature ML: report data sufficiency before any model is trained."""
    st.markdown("## 🧠 Phase 6 — Model readiness")
    frame, error = phase_1b_evidence_frame(index_name)
    if error or frame.empty:
        st.info("Collecting data: no settled directional outcomes are available for model research yet.")
        return
    events = frame.drop_duplicates("Event").copy()
    dates = pd.to_datetime(events["Entry time"], errors="coerce", utc=True).dt.date.nunique()
    regimes = events.groupby("Regime")["Event"].nunique() if "Regime" in events else pd.Series(dtype=int)
    horizon_15 = frame[frame["Horizon"] == 15]
    gates = [
        ("Independent state events", events["Event"].nunique(), 500),
        ("Separate market days", int(dates or 0), 20),
        ("15-minute settled labels", len(horizon_15), 500),
        ("Regimes with 50+ events", int((regimes >= 50).sum()), 3),
    ]
    ready = all(actual >= required for _, actual, required in gates)
    st.metric("ML status", "READY FOR WALK-FORWARD RESEARCH" if ready else "NOT READY — KEEP RECORDING")
    st.dataframe(pd.DataFrame(gates, columns=["Requirement", "Current", "Minimum"]).assign(
        Status=lambda d: np.where(d["Current"] >= d["Minimum"], "PASS", "COLLECTING")
    ), use_container_width=True, hide_index=True)
    st.caption("The thresholds are safeguards, not proof of profitability. Training must use day-based walk-forward validation with purging/embargo; random train/test splits are not permitted.")
    st.download_button(
        "Download normalized outcome research data", frame.to_csv(index=False).encode(),
        f"{index_name.replace(' ', '_').lower()}_phase6_research_outcomes.csv", "text/csv",
        key=f"phase6_export_{index_name}",
    )

# ---------------- PHASE 2: MULTI-HORIZON DECISION ENGINE ----------------
def signal_freshness(name, spot, scenario, levels, market, regime, phase, step):
    """Track persistence of a decision state and decay execution confidence.

    The live scenario itself is left untouched. Decay is an execution guard
    because an unchanged intraday state becomes less actionable with time.
    """
    score = float(scenario.get("score", 0) or 0)
    vwap = float(market.get("vwap", 0) or 0)
    bucket_size = max(float(step or 1) * 0.25, 1.0)
    level_bucket = lambda value: round(float(value or 0) / bucket_size) * bucket_size
    fingerprint = "|".join((
        str(scenario.get("primary", "RANGE / WAIT")), str(regime.get("regime", "UNKNOWN")), str(phase),
        str(int(round(score / 10.0) * 10)), "ABOVE_VWAP" if vwap and spot > vwap else "BELOW_VWAP" if vwap else "NO_VWAP",
        str(round(level_bucket(levels.get("eos_precision") or levels.get("eos")), 2)),
        str(round(level_bucket(levels.get("eor_precision") or levels.get("eor")), 2)),
    ))
    now = datetime.now(IST)
    key = f"signal_freshness_{name}"
    prior = st.session_state.get(key)
    if not prior or prior.get("fingerprint") != fingerprint:
        prior = {"fingerprint": fingerprint, "started_at": now.isoformat(), "changes": int((prior or {}).get("changes", 0)) + 1}
        st.session_state[key] = prior
    started = _event_time(prior.get("started_at")) or now
    age = max(0.0, (now - started).total_seconds() / 60.0)
    decay = 100.0 if age <= 5 else max(0.0, 100.0 * math.exp(-(age - 5.0) / 15.0))
    status = "FRESH" if age <= 5 else "AGING" if age <= 20 else "STALE"
    return {"age_minutes": age, "decay_pct": decay, "status": status, "fingerprint": fingerprint, "started_at": prior["started_at"], "changes": prior["changes"]}


def _current_evidence_bucket(score):
    value = abs(float(score or 0))
    return "0–24" if value < 25 else "25–49" if value < 50 else "50–74" if value < 75 else "75–100"

def _observed_direction_estimate(frame, direction, regime, score, horizon):
    """Return a gated historical observation, never a synthetic probability."""
    if direction not in ("UPSIDE", "DOWNSIDE") or frame.empty:
        return {"ready": False, "samples": 0, "scope": "not directional"}
    d = frame[(frame["Horizon"] == horizon) & (frame["Direction"] == direction) & frame["Correct"].notna()].copy()
    bucket = _current_evidence_bucket(score)
    # Prefer the most comparable group; relax only to report a broader, clearly
    # labelled baseline. A sparse group is never promoted to an estimate.
    candidates = [
        (d[(d["Regime"] == regime) & (d["Evidence bucket"] == bucket)], "same regime + score bucket"),
        (d[d["Evidence bucket"] == bucket], "same score bucket"),
        (d, "same direction baseline"),
    ]
    for sample, scope in candidates:
        if len(sample) >= 30:
            wins = int(sample["Correct"].astype(bool).sum())
            lo, hi = _wilson_interval(wins, len(sample))
            return {
                "ready": True, "samples": len(sample), "scope": scope,
                "hit_rate": wins / len(sample), "low": lo, "high": hi,
                "mean_abs_move_pct": sample["Raw return"].abs().mean(),
                "mean_return_pct": sample["Directional return"].mean(),
            }
    return {"ready": False, "samples": len(d), "scope": "collecting comparable evidence"}

def phase_2_decision_engine(index_name, spot, levels, scenario, market, regime, setup,
                            evidence_frame, eor_breakout, eos_breakdown, freshness=None):
    """Create a transparent decision state without changing the live signal model."""
    direction = str(scenario.get("primary", "RANGE / WAIT"))
    score = float(scenario.get("score", 0) or 0)
    horizon_rows = []
    for horizon in OUTCOME_HORIZONS:
        measured = _observed_direction_estimate(evidence_frame, direction, regime.get("regime", "UNKNOWN"), score, horizon)
        move_points = (spot * measured["mean_abs_move_pct"] / 100.0) if measured.get("ready") else None
        horizon_rows.append({
            "Horizon": f"{horizon}m", "Market state": direction,
            "Observed direction rate": measured.get("hit_rate") if measured.get("ready") else np.nan,
            "95% interval": f"{measured['low']:.1%} – {measured['high']:.1%}" if measured.get("ready") else "Collecting evidence",
            "Samples": measured.get("samples", 0), "Evidence scope": measured.get("scope"),
            "Observed move proxy": move_points,
            "Decision": "WATCH / CONFIRM" if measured.get("ready") and not setup.get("no_trade") else "NO TRADE / WAIT",
        })
    resistance = float(eor_breakout.get("level") or levels.get("eor") or spot)
    support = float(eos_breakdown.get("level") or levels.get("eos") or spot)
    proximity = min(abs(spot - resistance), abs(spot - support))
    data_ready = any(r["Samples"] >= 30 and not pd.isna(r["Observed direction rate"]) for r in horizon_rows)
    freshness = freshness or {}
    no_trade_reasons = list(setup.get("guard") or [])
    if direction not in ("UPSIDE", "DOWNSIDE"):
        no_trade_reasons.append("Current scenario is RANGE / WAIT.")
    if not data_ready:
        no_trade_reasons.append("Comparable historical outcomes are still being collected.")
    if freshness.get("status") == "STALE":
        no_trade_reasons.append("Signal state is stale; wait for refreshed alignment or a new state transition.")
    if not no_trade_reasons and setup.get("no_trade"):
        no_trade_reasons.append("The execution guard rejected the setup.")
    return {
        "direction": direction, "data_ready": data_ready,
        "no_trade": bool(no_trade_reasons), "no_trade_reasons": no_trade_reasons,
        "horizons": horizon_rows, "resistance": resistance, "support": support,
        "nearest_level_distance": proximity,
        "resistance_evidence": eor_breakout, "support_evidence": eos_breakdown, "freshness": freshness,
    }

def render_phase_2_decision_engine(index_name, spot, levels, scenario, market, regime, setup,
                                   eor_breakout, eos_breakdown, freshness=None):
    """Trader-facing Phase 2 view: conditional context, not an order ticket."""
    evidence_frame, evidence_error = phase_1b_evidence_frame(index_name)
    if evidence_error:
        evidence_frame = pd.DataFrame()
    decision = phase_2_decision_engine(
        index_name, spot, levels, scenario, market, regime, setup, evidence_frame,
        eor_breakout, eos_breakdown, freshness
    )
    st.markdown("## 🎯 Phase 2 — Multi-horizon decision map")
    a, b, c, d, e = st.columns(5)
    a.metric("Decision", "NO TRADE / WAIT" if decision["no_trade"] else "WATCH / CONFIRM")
    b.metric("Live state", decision["direction"])
    c.metric("Nearest calculated level", f"{decision['nearest_level_distance']:.1f} pts")
    d.metric("Historical evidence", "AVAILABLE" if decision["data_ready"] else "COLLECTING")
    fresh = decision.get("freshness") or {}
    e.metric("Signal freshness", f"{fresh.get('status', '—')} · {fresh.get('age_minutes', 0):.0f}m")
    st.caption("Historical direction rates appear only after 30 comparable settled events. They are observations, not guaranteed probabilities or trade instructions.")
    horizon_df = pd.DataFrame(decision["horizons"])
    st.dataframe(horizon_df.style.format({
        "Observed direction rate": "{:.1%}", "Observed move proxy": "{:.1f} pts",
    }, na_rep="—"), use_container_width=True, hide_index=True)
    st.caption("Observed move proxy = average absolute realized move from the comparable recorded sample. It is unavailable until the evidence gate is met.")
    if fresh:
        st.caption(f"Signal decay: {fresh.get('decay_pct', 0):.0f}% of its initial execution relevance. A state resets only when direction, regime, phase, score bucket, VWAP side, or calculated decision levels change.")

    x, y = st.columns(2)
    with x:
        st.markdown("### Resistance confirmation")
        st.metric(f"Calculated resistance · {decision['resistance']:,.2f}", f"{eor_breakout.get('probability', 0)} / 100 evidence")
        st.caption("Current live-evidence score only. Confirm a breakout only after a candle closes above the calculated level.")
    with y:
        st.markdown("### Support confirmation")
        st.metric(f"Calculated support · {decision['support']:,.2f}", f"{eos_breakdown.get('probability', 0)} / 100 evidence")
        st.caption("Current live-evidence score only. Confirm a breakdown only after a candle closes below the calculated level.")
    if decision["no_trade"]:
        st.warning("NO TRADE: " + " ".join(dict.fromkeys(decision["no_trade_reasons"])))
    else:
        st.success("WATCH / CONFIRM: historical evidence is available, but execution still requires the displayed trigger and candle-close confirmation.")
    return decision

# ---------------- PHASE 3: OI WALL INTELLIGENCE ----------------
WALL_OUTCOME_HORIZONS = (5, 15, 30, 60)

def _wall_behavior_snapshot(index_name, spot, levels, wall_rows, step, structure, migration):
    """Turn a live calculated OI wall into a reproducible behavior observation."""
    history = {f"{x.get('side')}:{float(x.get('strike', 0)):.4f}": x for x in (wall_rows or [])}
    snapshots = []
    migration_text = " | ".join(migration or [])
    for side in ("support", "resistance"):
        oi_key = "pe_oi" if side == "support" else "ce_oi"
        doi_key = "pe_doi_5m" if side == "support" else "ce_doi_5m"
        accel_key = "pe_accel" if side == "support" else "ce_accel"
        strength_key = "pe_strength" if side == "support" else "ce_strength"
        premium_key = "pe_prem_pct" if side == "support" else "ce_prem_pct"
        liquidity_key = "pe_liquidity" if side == "support" else "ce_liquidity"
        for row in (levels.get(side) or [])[:4]:
            structural = _json_number(row.get("structural", row.get("strike")))
            calculated = _json_number(row.get("precision", row.get("_calculated_level")))
            if structural is None or calculated is None:
                continue
            hist = history.get(f"{side}:{structural:.4f}", {})
            strength = _json_number(row.get(strength_key, row.get("_level_score"))) or 0.0
            oi = _json_number(row.get(oi_key)) or 0.0
            doi = _json_number(row.get(doi_key)) or 0.0
            accel = _json_number(row.get(accel_key)) or 0.0
            premium = _json_number(row.get(premium_key)) or 0.0
            liquidity = _json_number(row.get(liquidity_key)) or 0.0
            pulses = int(hist.get("observations", 0) or 0)
            tests = int(hist.get("tests", 0) or 0)
            reactions = int(hist.get("reactions", 0) or 0)
            oi_retention = oi / max(float(hist.get("max_oi", oi) or oi), 1.0)
            distance = abs(spot - calculated)
            # This is an explanatory 0–100 behavior score, not a probability.
            behavior_score = max(0.0, min(100.0,
                0.38 * strength + 18.0 * min(1.0, pulses / 12.0)
                + 12.0 * min(1.0, tests / 3.0) + 10.0 * min(1.0, max(oi_retention, 0.0))
                + 10.0 * min(1.0, max(liquidity, 0.0) / 100.0)
                + 7.0 * (1.0 if doi > 0 else 0.35 if doi == 0 else 0.0)
                + 5.0 * (1.0 if abs(premium) >= 0.5 else 0.35)
            ))
            if tests > 0 and distance <= max(step * 1.25, 1.0):
                state = "TESTING"
            elif pulses >= 6 and oi_retention >= 0.85:
                state = "PERSISTENT"
            elif oi_retention < 0.55 or doi < 0:
                state = "WEAKENING"
            else:
                state = "ACTIVE"
            snapshots.append({
                "index_name": index_name, "side": side, "structural_strike": structural,
                "calculated_level": calculated, "spot": float(spot), "strength": strength,
                "oi": oi, "doi_5m": doi, "oi_acceleration": accel,
                "premium_change_pct": premium, "liquidity": liquidity,
                "persistence_pulses": pulses, "test_count": tests, "reaction_count": reactions,
                "oi_retention": oi_retention, "distance_points": distance,
                "behavior_score": behavior_score, "state": state,
                "migration_context": migration_text,
                "near_test": distance <= max(step * 1.25, 1.0),
                "context": {"pcr": structure.get("pcr"), "ce_doi": structure.get("ce_doi"), "pe_doi": structure.get("pe_doi")},
            })
    return snapshots

def record_oi_wall_events(name, expiry, wall_snapshots, regime, phase):
    """Record only meaningful wall state transitions, not every live pulse."""
    if not supabase_enabled():
        return {"enabled": False, "recorded": 0}
    now = _supabase_minute()
    rows = []
    state_cache = st.session_state.setdefault(f"_oi_wall_event_fingerprints_{name}", {})
    for wall in wall_snapshots:
        # A wall earns an outcome label only when it is being tested or has
        # acquired enough persistence to be analytically meaningful.
        if not wall["near_test"] and wall["persistence_pulses"] < 6:
            continue
        fingerprint = "|".join((
            wall["side"], f"{wall['structural_strike']:.2f}", f"{wall['calculated_level']:.2f}",
            str(int(wall["behavior_score"] // 10) * 10), str(wall["persistence_pulses"] // 3),
            str(wall["test_count"]), wall["state"],
        ))
        key = f"{wall['side']}:{wall['structural_strike']:.4f}"
        if state_cache.get(key) == fingerprint:
            continue
        event_id = hashlib.sha256(f"{name}|{now.isoformat()}|{fingerprint}".encode("utf-8")).hexdigest()[:32]
        rows.append({
            "event_id": event_id, "observed_at": now.isoformat(), "trading_date": now.date().isoformat(),
            "index_name": name, "expiry": str(expiry) if expiry else None,
            "side": wall["side"], "structural_strike": wall["structural_strike"],
            "calculated_level": wall["calculated_level"], "spot": wall["spot"],
            "behavior_score": wall["behavior_score"], "state": wall["state"],
            "persistence_pulses": wall["persistence_pulses"], "test_count": wall["test_count"],
            "reaction_count": wall["reaction_count"], "distance_points": wall["distance_points"],
            "regime": str(regime.get("regime", "UNKNOWN")), "phase": str(phase),
            "wall_snapshot": wall,
        })
        state_cache[key] = fingerprint
    ok, count = _post_supabase("oi_wall_events", rows, "event_id")
    return {"enabled": True, "recorded": count if ok else 0}

def settle_oi_wall_outcomes(name, step):
    """Label hold/rejection, break, or no reaction from captured-minute prices.

    This research label never changes the chart's stricter candle-close break
    rule. It is explicitly marked close-sampled in the database and UI.
    """
    if not supabase_enabled():
        return {"enabled": False, "settled": 0, "pending": 0}
    today = datetime.now(IST).date().isoformat()
    events = _get_supabase("oi_wall_events", {
        "select": "event_id,observed_at,side,calculated_level,spot", "index_name": f"eq.{name}",
        "trading_date": f"eq.{today}", "order": "observed_at.asc", "limit": "500",
    })
    snapshots = _get_supabase("tradingcore_snapshots", {
        "select": "capture_minute,spot", "index_name": f"eq.{name}",
        "trading_date": f"eq.{today}", "order": "capture_minute.asc", "limit": "500",
    ...
    prices = [(t, _json_number(r.get("spot"))) for r in snapshots
              if (t := _event_time(r.get("capture_minute"))) is not None and _json_number(r.get("spot")) is not None]
    now, rows, pending = datetime.now(IST), [], 0
    for event in events:
        seen = _event_time(event.get("observed_at")); level = _json_number(event.get("calculated_level")); entry = _json_number(event.get("spot"))
        if seen is None or level is None or entry is None:
            continue
        path = [(t, p) for t, p in prices if t >= seen]
        for horizon in WALL_OUTCOME_HORIZONS:
            target = seen + timedelta(minutes=horizon)
            if now < target:
                pending += 1; continue
            future = [(t, p) for t, p in path if t >= target]
            if not future:
                pending += 1; continue
            settled_at, exit_price = future[0]
            if (settled_at - target).total_seconds() > 180:
                pending += 1; continue
            observed = [p for t, p in path if t <= settled_at]
            reaction = max(step * 0.50, abs(entry - level) * 0.50, 1.0)
            if event.get("side") == "support":
                broke = min(observed) < level
                held = not broke and exit_price >= entry + reaction
                move_away = exit_price - entry
            else:
                broke = max(observed) > level
                held = not broke and exit_price <= entry - reaction
                move_away = entry - exit_price
            outcome = "BREAK_CLOSE_SAMPLED" if broke else "HOLD_REJECTION" if held else "NO_REACTION"
            rows.append({
                "event_id": event["event_id"], "horizon_minutes": horizon,
                "settled_at": settled_at.isoformat(), "exit_price": exit_price,
                "outcome": outcome, "move_away_points": move_away,
                "max_favorable_points": max(observed) - entry if event.get("side") == "support" else entry - min(observed),
                "max_adverse_points": min(observed) - entry if event.get("side") == "support" else entry - max(observed),
                "observation_count": len(observed),
            })
    ok, count = _post_supabase("oi_wall_outcomes", rows, "event_id,horizon_minutes")
    return {"enabled": True, "settled": count if ok else 0, "pending": pending}

def _wall_reaction_rows(index_name, horizon=15):
    return _get_supabase("oi_wall_outcomes", {
        "select": "outcome,oi_wall_events!inner(index_name,side,behavior_score)",
        "oi_wall_events.index_name": f"eq.{index_name}",
        "horizon_minutes": f"eq.{horizon}", "limit": "2000",
    }) if supabase_enabled() else []

def _wall_reaction_evidence(index_name, side, behavior_score, horizon=15, history_rows=None):
    rows = history_rows if history_rows is not None else _wall_reaction_rows(index_name, horizon)
    bucket = int(float(behavior_score or 0) // 20) * 20
    matching = [r for r in rows if int(float((r.get("oi_wall_events") or {}).get("behavior_score") or 0) // 20) * 20 == bucket]
    if len(matching) < 30:
        return {"ready": False, "samples": len(matching), "bucket": bucket}
    counts = pd.Series([r.get("outcome", "NO_REACTION") for r in matching]).value_counts()
    return {
        "ready": True, "samples": len(matching), "bucket": bucket,
        "hold": float(counts.get("HOLD_REJECTION", 0) / len(matching)),
        "break": float(counts.get("BREAK_CLOSE_SAMPLED", 0) / len(matching)),
        "none": float(counts.get("NO_REACTION", 0) / len(matching)),
    }

def render_oi_wall_intelligence(index_name, wall_snapshots):
    st.markdown("## 🧱 Phase 3 — OI wall intelligence")
    st.caption("Behavior score combines strength, OI retention/change, persistence, tests, premium response and liquidity. Historical reactions use captured-minute prices; chart breaks still require a candle close.")
    rows = []
    history_rows = _wall_reaction_rows(index_name)
    for wall in wall_snapshots:
        evidence = _wall_reaction_evidence(index_name, wall["side"], wall["behavior_score"], history_rows=history_rows)
        rows.append({
            "Side": wall["side"].title(), "Calculated level": wall["calculated_level"],
            "Behavior": round(wall["behavior_score"]), "State": wall["state"],
            "Pulses": wall["persistence_pulses"], "Tests": wall["test_count"],
            "OI 5m": wall["doi_5m"], "Historical samples": evidence["samples"],
            "Hold / rejection": evidence.get("hold", np.nan), "Break": evidence.get("break", np.nan),
            "No reaction": evidence.get("none", np.nan),
        })
    if rows:
        st.dataframe(pd.DataFrame(rows).style.format({
            "Calculated level": "{:.2f}", "OI 5m": "{:+,.0f}",
            "Hold / rejection": "{:.1%}", "Break": "{:.1%}", "No reaction": "{:.1%}",
        }, na_rep="Collecting evidence"), use_container_width=True, hide_index=True)
    st.caption("A reaction percentage appears only after 30 comparable wall events. Before that, the system records behavior but does not claim a probability.")
...
def upstox_intraday_candles_df(token, key, minutes=5):
    """Build ONE continuous candle series: multi-month history + current-session data.

    Upstox V3 limits minute-history requests to roughly one month per request,
    so we stitch several bounded windows together and de-duplicate timestamps.
    The browser then appends the live tick to this same series.
    """
        ...
        for c in candles:
            ...
            try:
                if isinstance(ts, str):
                    dtt = _dt.datetime.fromisoformat(ts.replace("Z", "+00:00"))
                    epoch = int(dtt.timestamp())
                else:
                    epoch = int(float(ts))
                    if epoch > 10**12:
                        epoch //= 1000
    ...
    # Keep enough history to show days/weeks/months in the same chart.
    # 90 days gives ~3 months of context while keeping the browser responsive.
    history_days = 90 if int(minutes) <= 15 else 365
    all_frames = []

    # Current intraday endpoint is the fastest source for today's live session.
    try:
        d = api_get3(
    ...
    # V3 minute history is bounded per request. Stitch 30-day windows.
    if int(minutes) <= 15:
        chunk_days = 29
    else:
        chunk_days = 89
    ...
    cursor = today
    oldest = today - _dt.timedelta(days=history_days)
    while cursor >= oldest:
        chunk_from = max(oldest, cursor - _dt.timedelta(days=chunk_days))
        try:
            d = api_get3(
                f"/historical-candle/{quote(key, safe='')}/minutes/{int(minutes)}/"
                f"{cursor.isoformat()}/{chunk_from.isoformat()}",
                token,
            )
            frame = parse(d)
            if not frame.empty:
            ...
            pass
        cursor = chunk_from - _dt.timedelta(days=1)
    ...
    # Guard the browser against pathological payloads while preserving months
    # of 5m/15m history.  Never trim the current session away.
    max_rows = 50000
    if len(df) > max_rows:
        df = df.tail(max_rows).reset_index(drop=True)

    current_start = int(current["time"].min()) if not current.empty else 0
    if not current.empty:
        df.attrs["chart_status"] = "LIVE • CURRENT SESSION + 90D HISTORY"
    else:
        df.attrs["chart_status"] = "HISTORICAL • LIVE FEED WILL APPEND"
    df.attrs["session_date"] = today.isoformat()
    df.attrs["history_rows"] = len(df)
    df.attrs["history_start"] = int(df.iloc[0]["time"]) if not df.empty else 0
    df.attrs["current_start"] = current_start
    return df


def supabase_live_config_for_browser():
    return _supabase_browser_config()

def render_upstox_tv_chart(candles, spot, levels, name, tf_minutes=5, token='', live_symbol='', live_key=''):
    ...
    for side in ("support","resistance"):
        current=[x for x in life.get(side,[]) if not x.get("historical")][:4]
        broken=[x for x in life.get(side,[]) if x.get("historical") and x.get("state")=="BROKEN"][:1]
        for x in current+broken:
            price=float(x["precision"]); structural=float(x["structural"])
            key=(side,round(price,2),round(structural,2))
            if key in seen: continue
            seen.add(key)
            line_levels.append({"price":price,"structural":structural,"side":side,"state":x.get("state","ACTIVE"),"historical":bool(x.get("historical",False)),"tests":int(x.get("tests",0))})
    payload=_json.dumps(rows); lvl=_json.dumps(line_levels); nm=_json.dumps(str(name)); tf=_json.dumps(int(tf_minutes)); sb_url, sb_key = supabase_live_config_for_browser(); sb_url_json=_json.dumps(sb_url); sb_key_json=_json.dumps(sb_key); live_symbol_json=_json.dumps(str(live_symbol or name)); live_key_json=_json.dumps(str(live_key or live_symbol or '')); upstox_token_json=_json.dumps(str(token or ''))
...
#feedState{display:flex;align-items:center;gap:10px;min-height:30px;padding:4px 10px;border-bottom:1px solid #e5eaf0;background:#fff;font:10px system-ui;color:#64748b}
#feedState .state{font-weight:800;letter-spacing:.2px}
#feedState .meta{font-variant-numeric:tabular-nums}
#feedState.liveState .state{color:#087443}
#feedState.reconnectingState .state{color:#b45309}
#feedState.errorState .state{color:#b42318}
#feedState .dot{width:8px;height:8px;border-radius:50%;background:#087443;flex:0 0 8px}
#feedState.reconnectingState .dot{background:#f59e0b}
#feedState.errorState .dot{background:#dc2626}
#feedState .error{color:#b42318;max-width:48%;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
...
<div id="top"><div class="symbol">__NAME__</div><span class="live">● UPSTOX</span><span class="badge">OI levels <strong>LIVE</strong></span><span class="badge">Bar <strong id="barCountdown">--:--</strong></span><div class="spacer"></div><button onclick="fit()">Fit</button><button onclick="toggleFullscreen()">⛶</button></div><div id="feedState" class="reconnectingState"><span class="dot"></span><span class="state" id="feedStateText">🟠 RECONNECTING</span><span class="meta" id="feedLast">Last tick: —</span><span class="meta" id="feedAge">Age: —</span><span class="error" id="feedError"></span></div>
...
<script>
const DATA=__DATA__, LEVELS=__LEVELS__, TF_MINUTES=__TF_MINUTES__, LIVE_SYMBOL=__LIVE_SYMBOL__, LIVE_KEY=__LIVE_KEY__, UPSTOX_TOKEN=__UPSTOX_TOKEN__, KEY='nifty_tv_drawings_'+__NAME_JSON__;
...
function levelCloseBreaks(x,close){return x.side==='support'?close<x.price:close>x.price}function applyLevelStyle(entry){const x=entry.level,side=x.side==='support'?'S ':'R ',state=entry.broken?'BROKEN':x.state==='REVALIDATED'?'REVALIDATED':'ACTIVE',isBroken=state==='BROKEN',isRevalidated=state==='REVALIDATED';entry.line.applyOptions({color:isBroken?'#94a3b8':x.side==='support'?'#22c55e':'#ef4444',lineWidth:isBroken?1:2,lineStyle:isBroken?LightweightCharts.LineStyle.Dashed:LightweightCharts.LineStyle.Solid,axisLabelVisible:true,title:side+x.price.toFixed(2)+(isBroken?' · BROKEN CLOSE':isRevalidated?' · REVALIDATED':'')})}let levelSeries=LEVELS.map(x=>{const entry={level:x,broken:x.state==='BROKEN',line:cs.createPriceLine({price:x.price,axisLabelVisible:true})};applyLevelStyle(entry);return entry});function confirmLevelsOnCandleClose(close){levelSeries.forEach(entry=>{if(!entry.broken&&levelCloseBreaks(entry.level,close))entry.broken=true;applyLevelStyle(entry)})}function toggleLevels(){state.levels=!state.levels;document.getElementById('levelsBtn').classList.toggle('on',state.levels);levelSeries.forEach(s=>s.line.applyOptions({visible:state.levels}))}function toggleGrid(){state.grid=!state.grid;chart.applyOptions({grid:{vertLines:{visible:state.grid,color:'#172334'},horzLines:{visible:state.grid,color:'#172334'}}})}function toggleCrosshair(){state.crosshair=!state.crosshair;chart.applyOptions({crosshair:{mode:state.crosshair?LightweightCharts.CrosshairMode.Normal:LightweightCharts.CrosshairMode.Hidden}})}function setType(t){cs.applyOptions({visible:t==='candle'});ls.applyOptions({visible:t==='line'});document.getElementById('candleBtn').classList.toggle('on',t==='candle');document.getElementById('lineBtn').classList.toggle('on',t==='line')}function range(mins){if(!DATA.length)return;let end=DATA[DATA.length-1].time,start=end-mins*60;chart.timeScale().setVisibleRange({from:start,to:end});volChart.timeScale().setVisibleRange({from:start,to:end});pane.timeScale().setVisibleRange({from:start,to:end})}function fit(){chart.timeScale().fitContent();volChart.timeScale().fitContent();pane.timeScale().fitContent();renderDrawings()}function toggleFullscreen(){let x=document.getElementById('wrap');if(!document.fullscreenElement)x.requestFullscreen();else document.exitFullscreen()}
...
function point(ev){let r=mainEl.getBoundingClientRect(),x=ev.clientX-r.left,y=ev.clientY-r.top;return{t:chart.timeScale().coordinateToTime(x),p:cs.coordinateToPrice(y)}}mainEl.addEventListener('click',ev=>{if(!mode)return;let q=point(ev);if(!q.t||q.p==null)return;if(mode==='hline'){drawings.push({type:'hline',p:q.p});mode=null}else if(mode==='vline'){drawings.push({type:'vline',t:q.t});mode=null}else if(!pending)pending=q;else{drawings.push({type:mode,a:pending,b:q});pending=null;mode=null}drawMode(mode);renderDrawings()});function drawMode(m){mode=m;pending=null;document.querySelectorAll('.left button').forEach(x=>x.classList.remove('on'));document.getElementById(m||'cursor').classList.add('on');document.getElementById('modeLabel').textContent=m?m.toUpperCase():'Cursor';document.getElementById('hint').textContent=m?'Click '+(m==='hline'?'one price':m==='vline'?'one time point':'two points')+' on the chart.':'TradingView-style workspace • choose a drawing tool or indicator.'}function undo(){drawings.pop();renderDrawings()}function clearDrawings(){drawings=[];renderDrawings()}
 // Browser-side live bridge. The iframe owns the live candle.
 // Streamlit never needs to rerun for a tick.
 let liveBar=null,lastLiveAt=0,lastTickTs=0,followLive=true,lastServerPrice=0,feedHardError=false;
 function setFeedState(kind, source, price, error){
   const box=document.getElementById('feedState'),state=document.getElementById('feedStateText'),
         last=document.getElementById('feedLast'),err=document.getElementById('feedError');
   if(!box||!state)return;
   box.className=kind==='live'?'liveState':kind==='error'?'errorState':'reconnectingState';
   if(kind==='live'){
     state.textContent='🟢 LIVE';
     err.textContent='';
     if(price>0)err.title=''; 
   }else if(kind==='error'){
     state.textContent='🔴 FEED ERROR';
     err.textContent=String(error||'Unknown feed error');
     err.title=String(error||'Unknown feed error');
   }else{
     state.textContent='🟠 RECONNECTING';
     err.textContent=String(error||'');
     err.title=String(error||'');
   }
   if(lastLiveAt){
     last.textContent='Last tick: '+new Date(lastLiveAt).toLocaleTimeString('en-IN',{timeZone:'Asia/Kolkata',hour12:false});
   }else{
     last.textContent='Last tick: —';
   }
   if(price>0){
     const badge=document.querySelector('.live');
     if(badge){
       badge.textContent='● LIVE '+price.toLocaleString('en-IN',{maximumFractionDigits:2});
       badge.style.color=kind==='live'?'#087443':kind==='error'?'#b42318':'#b45309';
     }
   }
 }
 function updateLiveAge(){
   const el=document.getElementById('feedAge'); if(!el)return;
   if(!lastLiveAt){el.textContent='Age: —';return}
   const age=Math.max(0,(Date.now()-lastLiveAt)/1000);
   el.textContent='Age: '+age.toFixed(1)+'s';
   if(age>5 && age<=12 && !feedHardError)setFeedState('reconnecting','stale',lastServerPrice,'No tick received for '+age.toFixed(1)+'s');
   if(age>12 && !feedHardError)setFeedState('reconnecting','stale',lastServerPrice,'Feed silent for '+age.toFixed(1)+'s');
 }
 function updateLiveStatus(source,price,error){
   if(error){
     const msg=String(error);
     const auth=/HTTP[ ]*401|401|unauthorized|invalid token|access token/i.test(msg);
     feedHardError=auth;
     setFeedState(auth?'error':'reconnecting',source,0,msg);
     return;
   }
   feedHardError=false;
   const p=Number(price||0);
   if(p>0)setFeedState('live',source,p,'');
 }
 function updateBarCountdown(){
   const el=document.getElementById('barCountdown'); if(!el)return;
   const now=Math.floor(Date.now()/1000), bs=Math.max(60,Number(TF_MINUTES||5)*60);
   let remain=bs-(now%bs); if(remain===bs)remain=0;
   el.textContent=String(Math.floor(remain/60)).padStart(2,'0')+':'+String(remain%60).padStart(2,'0');
 }
 setInterval(updateBarCountdown,250);updateBarCountdown();

 let ema20Live=null,ema50Live=null,sma20Sum=0,sma50Sum=0;
 function seedIndicatorState(){
   if(!DATA.length)return;
   let e20=null,e50=null,s20=0,s50=0;
   DATA.forEach((x,i)=>{
     const c=Number(x.close);
     e20=e20==null?c:(2/21)*c+(19/21)*e20;
     e50=e50==null?c:(2/51)*c+(49/51)*e50;
     s20+=c;s50+=c;
     if(i>=20)s20-=Number(DATA[i-20].close);
     if(i>=50)s50-=Number(DATA[i-50].close);
   });
   ema20Live=e20;ema50Live=e50;sma20Sum=s20;sma50Sum=s50;
 }
 seedIndicatorState();
 mainEl.addEventListener('wheel',()=>{followLive=false},{passive:true});

 function applyLiveTick(t,source='live'){
   try{
     const price=Number(t.price??t.last_price??0);
     if(!(price>0))throw new Error('invalid live price');

     const receiptTs=Date.now();
     let ts=Number(t.ts??t.ltt??t.last_trade_time??0);
     if(!Number.isFinite(ts)||ts<=0)ts=receiptTs;
     if(ts<1e12)ts*=1000;
     ts=Math.floor(ts);
     const nowSec=Math.floor(receiptTs/1000);
     let sec=Math.floor(ts/1000);
     if(sec>nowSec+10)sec=nowSec;

     const bucketSeconds=Math.max(60,Number(TF_MINUTES||5)*60);
     const historicalLast=DATA.length?Number(DATA[DATA.length-1].time):0;

     // Do not throw away a current tick merely because the historical endpoint
     // returned a slightly newer bucket.  Only reject genuinely old data.
     if(historicalLast && sec < historicalLast-bucketSeconds*2)return;

     const bucket=sec-sec%bucketSeconds;
     const last=DATA.length?DATA[DATA.length-1]:null;

     if(!liveBar || Number(liveBar.time)!==bucket){
       // A level is broken only after the prior, completed chart candle closes
       // through its calculated precision price. Intrabar wicks/ticks do not count.
       if(liveBar)confirmLevelsOnCandleClose(Number(liveBar.close));
       const base=(last&&Number(last.time)===bucket)?last:null;
       liveBar={
         time:bucket,
         open:base?Number(base.open):price,
         high:base?Math.max(Number(base.high),price):price,
         low:base?Math.min(Number(base.low),price):price,
         close:price,
         volume:Number(t.volume??base?.volume??0)||0
       };
       if(!DATA.length || Number(DATA[DATA.length-1].time)<bucket) DATA.push(liveBar);
       else DATA[DATA.length-1]=liveBar;
     }else{
       liveBar.close=price;
       liveBar.high=Math.max(Number(liveBar.high||price),price);
       liveBar.low=Math.min(Number(liveBar.low||price),price);
       if(Number(t.volume)>0)liveBar.volume=Number(t.volume);
       if(DATA.length)DATA[DATA.length-1]=liveBar;
     }

     // THIS is the only price-series write.  Never recreate the chart.
     cs.update({
       time:Number(liveBar.time),
       open:Number(liveBar.open),
       high:Number(liveBar.high),
       low:Number(liveBar.low),
       close:Number(liveBar.close)
     });
     ls.update({time:Number(liveBar.time),value:Number(price)});

     const c=Number(liveBar.close);
     ema20Live=ema20Live==null?c:(2/21)*c+(19/21)*ema20Live;
     ema50Live=ema50Live==null?c:(2/51)*c+(49/51)*ema50Live;
     sma20Sum+=c;sma50Sum+=c;
     if(DATA.length>20)sma20Sum-=Number(DATA[DATA.length-2].close);
     if(DATA.length>50)sma50Sum-=Number(DATA[DATA.length-2].close);
     if(overlays.ema20)overlays.ema20.update({time:Number(liveBar.time),value:ema20Live});
     if(overlays.ema50)overlays.ema50.update({time:Number(liveBar.time),value:ema50Live});
     if(overlays.sma20)overlays.sma20.update({time:Number(liveBar.time),value:sma20Sum/Math.min(20,DATA.length)});
     if(overlays.sma50)overlays.sma50.update({time:Number(liveBar.time),value:sma50Sum/Math.min(50,DATA.length)});
     vol.update({time:Number(liveBar.time),value:Number(liveBar.volume||0)});
     lastLiveAt=receiptTs;
     lastTickTs=Math.max(lastTickTs,ts);
     lastServerPrice=price;
     if(followLive)chart.timeScale().scrollToRealTime();
     updateLiveStatus(source,price);
   }catch(e){
     updateLiveStatus(source,0,e.message||e);
     console.error('TradingCoreOS live candle update failed',e,t);
   }
 }

 // Supabase is an optional recorder/bridge. A missing table (404) must not
 // cause a request every second or hold the candle engine hostage.
 let supabaseRetryAt=0,supabasePollBusy=false;
 async function supabaseLatest(){
   const SB_URL=__SB_URL__,SB_KEY=__SB_KEY__;
   if(!SB_URL||!SB_KEY||Date.now()<supabaseRetryAt||supabasePollBusy)return false;
   supabasePollBusy=true;
   try{
     const url=SB_URL+'/rest/v1/live_ticks?symbol=eq.'+encodeURIComponent(LIVE_SYMBOL)+
       '&select=ts,price&order=ts.desc&limit=1';
     const r=await fetch(url,{headers:{apikey:SB_KEY,Authorization:'Bearer '+SB_KEY},cache:'no-store'});
     if(!r.ok){
       // A 404 normally means the live_ticks table/migration is absent. Back
       // off for five minutes rather than repeatedly hammering a bad endpoint.
       supabaseRetryAt=Date.now()+(r.status===404?300000:30000);
       throw new Error('Supabase HTTP '+r.status+' — chart recovery continuing');
     }
     const a=await r.json();
     if(!a||!a.length)return false;
     const ts=new Date(a[0].ts).getTime();
     const price=Number(a[0].price||0);
     if(price>0 && ts>lastTickTs){
       applyLiveTick({price,ts},'supabase');
       return true;
     }
     // A row existing in Supabase is NOT the same thing as a fresh tick.
     // If it is older than the last tick received by this iframe, return
     // false so the one-second Upstox V3 LTP fallback can fetch the current
     // market price immediately.
     return false;
   }catch(e){
     // Supabase is optional transport. Do not mark the feed dead here;
     // the direct Upstox fallback remains authoritative.
     setFeedState('reconnecting','supabase',lastServerPrice,'Supabase: '+(e.message||e));
     return false;
   }finally{supabasePollBusy=false;}
 }

 // Direct Upstox REST is emergency-only. The normal path is:
 // Upstox V3 WebSocket -> Python -> Supabase -> this iframe.
 let directPollBusy=false,lastDirectPoll=0,directRetryAt=0,directBackoffMs=2500;
 async function pollUpstoxDirect(){
   // REST is recovery-only. It is deliberately slower than the WebSocket
   // bridge and backs off on 429 so it cannot become the source of a freeze.
   if(Date.now()<directRetryAt||Date.now()-lastDirectPoll<2500)return false;
   if(!UPSTOX_TOKEN||!LIVE_KEY||directPollBusy)return false;
   directPollBusy=true;lastDirectPoll=Date.now();
   try{
     const u='https://api.upstox.com/v3/market-quote/ltp?instrument_key='+encodeURIComponent(LIVE_KEY);
     const r=await fetch(u,{headers:{Accept:'application/json',Authorization:'Bearer '+UPSTOX_TOKEN},cache:'no-store'});
     if(!r.ok){
       if(r.status===429){
         const retryAfter=Number(r.headers.get('Retry-After')||0)*1000;
         directBackoffMs=Math.min(Math.max(retryAfter,directBackoffMs*2),60000);
         directRetryAt=Date.now()+directBackoffMs;
       }else{
         directRetryAt=Date.now()+Math.min(directBackoffMs,15000);
       }
       throw new Error('Upstox V3 HTTP '+r.status);
     }
     const body=await r.json(),data=body?.data||{};
     const q=data[LIVE_KEY]||data[LIVE_KEY.replace('|',':')]||data[Object.keys(data)[0]];
     const price=Number(q?.last_price||0);
     if(price>0){
       applyLiveTick({price,ts:Date.now(),volume:Number(q?.volume||0)},'upstox-rest');
       directBackoffMs=2500;
       directRetryAt=Date.now()+2500;
       return true;
     }
   }catch(e){
     const msg=String(e?.message||e);
     updateLiveStatus('upstox-rest',0,msg);
     console.warn('Upstox browser fallback failed',e);
   }finally{directPollBusy=false}
   return false;
 }
 ...
 async function startRealtime(){
         ...
         const channel=client.channel(channelName)
           .on('postgres_changes',{event:'INSERT',schema:'public',table:'live_ticks'},
             payload=>{
               ...
               if(row&&row.symbol===LIVE_SYMBOL)applyLiveTick(row,'supabase-realtime');
             })
           .subscribe(status=>{
             if(status==='SUBSCRIBED')updateLiveStatus('supabase',lastServerPrice);
           });
   ...
   // Initial catch-up. The recurring path below is deliberately cheap.
   if(!(await supabaseLatest()))await pollUpstoxDirect();
 }

 // No Streamlit timer/rerun. Only this iframe's browser transport runs.
 // Supabase is the first transport, but it is NEVER allowed to block the
 // direct Upstox path when its newest row is stale. This is the key freeze fix.
 setInterval(async()=>{
   const got=await supabaseLatest();
   if(!got) await pollUpstoxDirect();
   updateLiveAge();
 },1000);
 setInterval(updateLiveAge,250);
 startRealtime();
 ...
 addLine('ema20',ema(20),'#60a5fa');setPane('RSI');fit();renderDrawings();new ResizeObserver(()=>{chart.applyOptions({width:mainEl.clientWidth});pane.applyOptions({width:paneEl.clientWidth});volChart.applyOptions({width:volEl.clientWidth});renderDrawings()}).observe(mainEl);chart.timeScale().subscribeVisibleTimeRangeChange(()=>{renderDrawings();try{let r=chart.timeScale().getVisibleRange();if(r){volChart.timeScale().setVisibleRange(r);pane.timeScale().setVisibleRange(r)}}catch(e){}});
    ...
    html=html.replace('__DATA__',payload).replace('__LEVELS__',lvl).replace('__TF_MINUTES__',tf).replace('__NAME__',str(name).replace('&','&amp;').replace('<','&lt;').replace('>','&gt;')).replace('__NAME_JSON__',nm).replace('__SB_URL__',sb_url_json).replace('__SB_KEY__',sb_key_json).replace('__LIVE_SYMBOL__',live_symbol_json).replace('__LIVE_KEY__',live_key_json).replace('__UPSTOX_TOKEN__',upstox_token_json)
        ...
        if not d.empty:
            ...
            for side in ("support","resistance"):
                for x in payload[side]:
                    val=x["precision"]
                    broken=x["state"]=="BROKEN"
                    label=("BROKEN " if broken else "") + ("S" if side=="support" else "R") + f" {val:,.2f}"
                    fig.add_hline(
                        y=val, line_width=1 if broken else 2,
                        line_color="#94a3b8" if broken else ("#22c55e" if side=="support" else "#ef4444"),
                        line_dash="dot" if broken else "solid",
...
def _level_lifecycle_engine(levels, spot, df=None, now=None):
    ...
    # Server-side lifecycle changes must use a completed candle close when an
    # OHLC frame is available. The browser chart enforces the same rule from
    # its just-completed live bar; a bare live tick is never a confirmation.
    confirmed_close=None
    try:
        if df is not None and not getattr(df,"empty",True):
            close_col=next((c for c in ("close","Close") if c in df.columns),None)
            if close_col:
                confirmed_close=_safe_num(df[close_col].iloc[-1])
    except Exception:
        confirmed_close=None
    ...
    if isinstance(levels, dict):
        ...
        for k,v in levels.items():
            ...
            if isinstance(v,(int,float,np.integer,np.floating)):
                # EOS/EOR numeric anchors are structural round strikes. They
                # remain available to analytics but must never become chart
                # lines when the user requests calculated levels only.
                continue
    ...
    for d in candidates:
        ...
        wall_score=_safe_num(d.get("_level_score", d.get("wall_score")), 0.0) or 0.0
        if structural is None:
            continue

        # The calculated premium-derived price is the tradable level. Keeping
        # lifecycle state by strike incorrectly carries a break at (say) 24200
        # forward to its later calculated support at 24187.
        key=f"{side}:{round(precision,2)}"
        ...
        broken=(confirmed_close < precision) if confirmed_close is not None and side=="support" else (confirmed_close > precision) if confirmed_close is not None else False
        ...
        if near and not broken:
            ...
            if state["state"] not in ("BROKEN","FLIPPED"):
                state["state"]="BROKEN"
                state["break_count"] += 1
                state["break_strength"]=wall_score
        ...
        if state["state"]=="BROKEN":
            # A broken level is re-armed only when the OI wall itself rebuilds
            # materially and price has reclaimed the calculated level. A mere
            # price bounce never turns a gray broken level back to green.
            reclaimed=(spot >= precision+break_buffer) if side=="support" else (spot <= precision-break_buffer)
            break_strength=float(state.get("break_strength",0.0) or 0.0)
            strength_rebuilt=wall_score >= max(60.0, break_strength+5.0)
            if reclaimed and strength_rebuilt:
                state["state"]="REVALIDATED"
                state["history"].append({"time":now,"event":"REVALIDATED","price":spot})
    ...
    # Retain recently broken calculated prices as gray historical lines. This
    # gives a trader a clear break/retest reference before the level expires.
    ...
    for key,state in list(store.items()):
        ...
        if age>history_seconds:
            store.pop(key,None)
            continue
        if state.get("state")!="BROKEN" or not state.get("precision"):
            continue
        side=state.get("side","support")
        if any(round(x["precision"],2)==round(float(state["precision"]),2) for x in out[side]):
            continue
        out[side].append({
            "side":side,
            "structural":float(state.get("structural",state["precision"])),
            "precision":float(state["precision"]),
            "state":"BROKEN",
            "distance":abs(spot-float(state["precision"])),
            "tests":int(state.get("tests",0)),
            "break_count":int(state.get("break_count",0)),
            "historical":True,
        })

    # Current calculated levels rank first; historical broken levels follow.
    out["support"].sort(key=lambda x: (x.get("historical",False), abs(spot-x["precision"])))
    out["resistance"].sort(key=lambda x: (x.get("historical",False), abs(spot-x["precision"])))
...
SECTOR_BY_SYMBOL = {
    "HDFCBANK":"Financials", "ICICIBANK":"Financials", "AXISBANK":"Financials", "SBIN":"Financials", "KOTAKBANK":"Financials", "BAJFINANCE":"Financials", "SHRIRAMFIN":"Financials", "FEDERALBNK":"Financials", "INDUSINDBK":"Financials", "AUBANK":"Financials", "BANKBARODA":"Financials", "IDFCFIRSTB":"Financials", "PNB":"Financials", "CANBK":"Financials", "UNIONBANK":"Financials", "YESBANK":"Financials",
    "RELIANCE":"Energy", "NTPC":"Energy", "POWERGRID":"Utilities",
    "INFY":"IT", "TCS":"IT", "HCLTECH":"IT",
    "BHARTIARTL":"Telecom", "LT":"Industrials", "BEL":"Industrials",
    "ITC":"FMCG", "HINDUNILVR":"FMCG", "M&M":"Auto", "MARUTI":"Auto",
    "SUNPHARMA":"Pharma", "TITAN":"Consumer", "ETERNAL":"Consumer",
    "TATASTEEL":"Metals", "JSWSTEEL":"Metals", "HINDALCO":"Metals", "ULTRACEMCO":"Materials",
}
...
def api_get3(path, token, params=None, timeout=15):
    ...
    if not r.ok:
        ...
        raise RuntimeError(f"Upstox V3 HTTP {r.status_code}{retry_note}: {body}")
    return body
...
def previous_session_close(token, key):
    ...
    for back in range(1, 8):
        ...
        try:
            ...
            candles = ((d.get("data") or {}).get("candles") or []) if isinstance(d, dict) else []
            if candles:
                # Upstox may return candles newest-first. The last list entry
                # is therefore not reliably the session close and can make the
                # daily percentage disagree with TradingView. Select the row
                # with the latest timestamp explicitly.
                latest=max(
                    (c for c in candles if len(c)>=5),
                    key=lambda c: pd.to_datetime(c[0],utc=True,errors="coerce").value,
                    default=None,
                )
...
def pick_levels(d, spot, step, side, n=4):
    ...
    # A PE/CE wall's actionable line is its calculated premium-derived price,
    # not the round option strike. For example, a 24,200 PE wall can generate
    # a 24,182 support and must remain eligible while spot is above 24,182.
    src=d.copy()
    src["_calculated_level"]=[
        _precision_reversal(row, side)[0]
        for _,row in src.iterrows()
    ]
    # Never substitute the round strike if the premium is unavailable. The
    # chart is deliberately calculated-level-only, even when that means fewer
    # than four levels are available from the live option chain.
    src=src[pd.to_numeric(src["_calculated_level"],errors="coerce").notna()].copy()
    src["_calculated_level"]=pd.to_numeric(src["_calculated_level"],errors="coerce")
    if src.empty:
        return []
    src=src[src["_calculated_level"] > spot].copy() if side == "resistance" else src[src["_calculated_level"] < spot].copy()
    if src.empty:
        return []
    ...
    primary = src[(src._calculated_level - spot).abs() <= 8.0 * step].copy()
    if primary.empty:
        primary = src[(src._calculated_level - spot).abs() <= 12.0 * step].copy()
    if primary.empty:
        return []
    ...
    dist = (candidates["_calculated_level"] - spot).abs()
    ...
    candidates["_near_score"] = np.exp(-(candidates["_calculated_level"] - spot).abs() / max(1.75 * step, 1.0))
    ...
    for _, row in candidates.iterrows():
        if all(abs(float(row._calculated_level) - float(q._calculated_level)) >= min_gap for q in chosen):
            chosen.append(row)
        if len(chosen) >= n:
            break
...
def calculate_levels(df, spot, step):
    ...
    def primary(rows, side):
        ...
        near = [r for r in rows if abs(float(r.get("_calculated_level",r.strike)) - spot) <= 2.25 * step]
        if near:
    ...
    return {
        ...
        "support": [
            {**r.to_dict(), "side":"support", "structural":float(r.strike),
             "precision": float(r["_calculated_level"])}
            for r in sup
        ],
        "resistance": [
...
def render_contributors(token, name):
    ...
    df["sector"] = df.symbol.map(SECTOR_BY_SYMBOL).fillna("Other")
    sectors = df.groupby("sector").agg(
        impact=("impact", "sum"), advancing=("move", lambda s: int((s > 0).sum())),
        members=("symbol", "size"), average_move=("move", "mean"),
    ).reset_index().sort_values("impact", ascending=False)
    context = {
        "tracked": len(df), "advancing": adv, "declining": dec, "net_impact": float(weighted),
        "top3_share": float(top3_share), "participation": quality,
        "leaders": sectors.head(3).to_dict("records"), "laggards": sectors.tail(3).sort_values("impact").to_dict("records"),
    }
    st.session_state[f"breadth_context_{name}"] = context
    st.markdown(f"## Who is moving {name}?")
    st.caption("Index impact = constituent index weight × stock move. This is contribution, not a trade signal.")
    ...
    l,r=st.columns(2)
    with l:
        st.markdown("### 🟢 NIFTY PULL" if name=="NIFTY 50" else "### 🟢 INDEX PULL")
        for _,row in top_up.head(4).iterrows():
            st.markdown(f"🟢 **{row['symbol']:<11}** {row['move']:+.3f}% · impact {row['impact']:+.3f}%")
    ...
    with r:
        st.markdown("### 🔴 NIFTY DRAG" if name=="NIFTY 50" else "### 🔴 INDEX DRAG")
        for _,row in top_dn.head(4).iterrows():
            st.markdown(f"🔴 **{row['symbol']:<11}** {row['move']:+.3f}% · impact {row['impact']:+.3f}%")
    ...
    st.dataframe(sectors.rename(columns={"sector":"Sector","impact":"Index impact %","advancing":"Up","members":"Tracked","average_move":"Average move %"}).round(3),use_container_width=True,hide_index=True)
...
def market_context_read(name, spot, market, df, structure, expiry, expected_move_points, breadth_context=None):
    """Phase 4 context: breadth and volatility state, kept separate from prediction."""
    breadth_context = breadth_context or st.session_state.get(f"breadth_context_{name}", {})
    session_range = max(0.0, float(market.get("high", spot) or spot) - float(market.get("low", spot) or spot))
    expected = float(expected_move_points or 0)
    nearby = strike_window(df, spot, max(1.0, abs(spot) * 0.001), 7) if df is not None and not df.empty else pd.DataFrame()
    if nearby.empty:
        oi_concentration = 0.0
    else:
        oi = nearby.get("ce_oi", pd.Series(dtype=float)).fillna(0).clip(lower=0) + nearby.get("pe_oi", pd.Series(dtype=float)).fillna(0).clip(lower=0)
        oi_concentration = float(oi.max() / max(float(oi.sum()), 1.0))
    ret5, ret15 = abs(float(market.get("ret5", 0) or 0)), abs(float(market.get("ret15", 0) or 0))
    if expected > 0 and session_range >= expected * 1.05 and max(ret5, ret15) >= 0.12:
        volatility_state = "VOLATILITY EXPANSION"
    elif expected > 0 and session_range <= expected * 0.60 and max(ret5, ret15) <= 0.10 and oi_concentration >= 0.18:
        volatility_state = "COMPRESSION BUILDING"
    else:
        volatility_state = "NORMAL / TRANSITION"
    expiry_days = None
    try:
        expiry_days = (date.fromisoformat(str(expiry)[:10]) - datetime.now(IST).date()).days
    except Exception:
        pass
    expiry_state = "EXPIRY DAY" if expiry_days == 0 else "NEAR EXPIRY" if expiry_days is not None and expiry_days <= 1 else "NORMAL EXPIRY CONTEXT"
    participation = breadth_context.get("participation", "UNAVAILABLE")
    concentration = float(breadth_context.get("top3_share", 0) or 0)
    if participation == "NARROW" or concentration >= 0.60:
        breadth_state = "NARROW / HEAVYWEIGHT-LED"
    elif participation == "BROAD":
        breadth_state = "BROAD PARTICIPATION"
    else:
        breadth_state = "MIXED PARTICIPATION"
    return {
        "breadth_state": breadth_state, "participation": participation, "concentration": concentration,
        "volatility_state": volatility_state, "session_range": session_range,
        "expected_move": expected, "oi_concentration": oi_concentration,
        "expiry_state": expiry_state, "expiry_days": expiry_days,
        "leaders": breadth_context.get("leaders", []), "laggards": breadth_context.get("laggards", []),
    }


def render_phase_4_market_context(context):
    st.markdown("## 🌐 Phase 4 — Market context")
    a, b, c, d = st.columns(4)
    a.metric("Breadth quality", context["breadth_state"])
    b.metric("Top-3 contribution", f"{context['concentration']:.0%}" if context["participation"] != "UNAVAILABLE" else "—")
    c.metric("Volatility state", context["volatility_state"])
    d.metric("Expiry", context["expiry_state"])
    st.caption(f"Session range {context['session_range']:.1f} pts • ATM straddle movement proxy {context['expected_move']:.1f} pts • nearby OI concentration {context['oi_concentration']:.0%}. These are context measurements, not a direction forecast.")
    leaders = context.get("leaders") or []
    laggards = context.get("laggards") or []
    if leaders or laggards:
        l, r = st.columns(2)
        with l:
            st.markdown("### Sector leadership")
            if leaders:
                st.dataframe(pd.DataFrame(leaders).rename(columns={"sector":"Sector", "impact":"Index impact %", "advancing":"Up", "members":"Tracked", "average_move":"Average move %"}).round(3), use_container_width=True, hide_index=True)
            else:
                st.caption("Constituent quote coverage is unavailable.")
        with r:
            st.markdown("### Sector drag")
            if laggards:
                st.dataframe(pd.DataFrame(laggards).rename(columns={"sector":"Sector", "impact":"Index impact %", "advancing":"Up", "members":"Tracked", "average_move":"Average move %"}).round(3), use_container_width=True, hide_index=True)
            else:
                st.caption("Constituent quote coverage is unavailable.")


# ---------------- PHASE 5: OPTION EXPRESSION LENS ----------------
def option_expression_engine(spot, df, step, scenario, setup, decision, structure, expected_move_points):
    """Connect an underlying setup to a transparent option-quality check.

    This is deliberately an execution lens, not personalized investment advice
    or an automatic order generator. A clean underlying view does not by itself
    make buying an option attractive.
    """
    direction = scenario.get("primary", "RANGE / WAIT")
    if df is None or df.empty:
        return {"status": "NO TRADE", "reasons": ["Option-chain quotes are unavailable."], "direction": direction}
    side = "ce" if direction == "UPSIDE" else "pe" if direction == "DOWNSIDE" else None
    if side is None:
        return {"status": "NO TRADE", "reasons": ["Underlying state is RANGE / WAIT."], "direction": direction}
    # Use the liquid-attempt ATM contract as a reference. The tool does not
    # silently substitute a round strike for a calculated price level.
    candidates = df.copy()
    candidates["_dist"] = (pd.to_numeric(candidates["strike"], errors="coerce") - spot).abs()
    row = candidates.sort_values("_dist").iloc[0]
    strike = _json_number(row.get("strike"))
    ltp = _json_number(row.get(f"{side}_ltp")) or 0.0
    bid = _json_number(row.get(f"{side}_bid")) or 0.0
    ask = _json_number(row.get(f"{side}_ask")) or 0.0
    iv = _json_number(row.get(f"{side}_iv"))
    volume = _json_number(row.get(f"{side}_vol")) or 0.0
    doi = _json_number(row.get(f"{side}_doi_5m")) or 0.0
    premium_change = _json_number(row.get(f"{side}_prem_pct")) or 0.0
    spread_pct = ((ask - bid) / ltp * 100.0) if ltp > 0 and bid > 0 and ask >= bid else None
    liquidity = "OK" if spread_pct is not None and spread_pct <= 1.2 else "WATCH" if spread_pct is not None and spread_pct <= 2.5 else "POOR / UNKNOWN"
    reasons = []
    if decision.get("no_trade"):
        reasons.append("Underlying decision engine is in NO TRADE / WAIT.")
    if not decision.get("data_ready"):
        reasons.append("Comparable underlying outcomes are still being collected.")
    if setup.get("no_trade"):
        reasons.extend(setup.get("guard") or ["Underlying execution guard is active."])
    if ltp <= 0:
        reasons.append("Reference option has no valid traded premium.")
    if liquidity != "OK":
        reasons.append("Bid/ask liquidity is not clean enough for a directional premium idea.")
    if expected_move_points and ltp > expected_move_points * 0.85:
        reasons.append("ATM premium is large versus the current straddle movement proxy.")
    status = "NO TRADE" if reasons else "RESEARCH CANDIDATE"
    expression = "CE directional premium" if side == "ce" else "PE directional premium"
    if iv is not None and iv >= 0.25:
        expression = "Debit-spread research candidate" if status != "NO TRADE" else expression
    return {
        "status": status, "direction": direction, "option_side": side.upper(), "expression": expression,
        "strike": strike, "ltp": ltp, "bid": bid, "ask": ask, "spread_pct": spread_pct,
        "iv": iv, "volume": volume, "doi_5m": doi, "premium_change_pct": premium_change,
        "liquidity": liquidity, "reasons": list(dict.fromkeys(reasons)),
        "trigger": setup.get("trigger"), "target": setup.get("target"), "invalidation": setup.get("stop"),
    }


def render_phase_5_option_expression(expression):
    st.markdown("## 🧩 Phase 5 — Option expression lens")
    a, b, c, d = st.columns(4)
    a.metric("Expression state", expression["status"])
    b.metric("Underlying direction", expression.get("direction", "—"))
    c.metric("Reference contract", f"{expression.get('strike', 0):,.0f} {expression.get('option_side', '')}" if expression.get("strike") else "—")
    d.metric("Liquidity", expression.get("liquidity", "—"))
    if expression.get("strike"):
        x, y, z, w = st.columns(4)
        x.metric("LTP", f"₹{expression.get('ltp', 0):.2f}")
        y.metric("Bid / ask", f"₹{expression.get('bid', 0):.2f} / ₹{expression.get('ask', 0):.2f}")
        z.metric("Spread", f"{expression['spread_pct']:.2f}%" if expression.get("spread_pct") is not None else "—")
        w.metric("Premium / 5m OI", f"{expression.get('premium_change_pct', 0):+.2f}% / {expression.get('doi_5m', 0):+,.0f}")
    if expression["status"] == "NO TRADE":
        st.warning("NO TRADE: " + " ".join(expression.get("reasons") or ["Execution conditions are insufficient."]))
    else:
        st.info(f"{expression['expression']}: validate the underlying trigger at {float(expression.get('trigger') or 0):,.2f}, target {float(expression.get('target') or 0):,.2f}, and invalidation {float(expression.get('invalidation') or 0):,.2f} before any manual order.")
    st.caption("Index direction is not an option-buy signal. This lens checks live premium and liquidity, but does not estimate transaction costs, slippage, tax, or personal risk capacity.")
...
def session_market_metrics(token, key):
    ...
    try:
        ...
        first15 = x.iloc[:15] if len(x) >= 15 else x
        log_returns = np.log(x.close / x.close.shift(1)).replace([np.inf, -np.inf], np.nan).dropna()
        annualize = lambda r: float(r.std(ddof=1) * math.sqrt(375 * 252)) if len(r) >= 10 else np.nan
        return {
            **d,
            "vwap": vwap,
            ...
            "bars": len(x),
            "realized_vol_session": annualize(log_returns),
            "realized_vol_30m": annualize(log_returns.tail(30)),
...
def market_state_vector(spot, levels, market, structure, futures, regime, phase, breadth_score=0.0):
    """Normalized one-minute state for research and decision context, not a trade order."""
    clip=lambda v: float(max(-1.0,min(1.0,float(v))))
    ...
    oi_pressure=clip((dpe-dce)/max(abs(dpe)+abs(dce),1.0))
    liquidity=0.0
    try:
        liquidity=float(np.nanmean([structure.get("ce_liquidity",0),structure.get("pe_liquidity",0)]))/100.0
    except Exception:
        pass
    return {
        "trend":clip(r15*4.0),
        "momentum":clip(r5*8.0+r15*2.0),
        "volatility":clip((float(market.get("high",spot) or spot)-float(market.get("low",spot) or spot)) / max(float(spot),1.0)*25.0),
        "vwap_relationship":clip((spot-vwap)/max(abs(spot)*0.0015,1.0)) if vwap else 0.0,
        "opening_range":1.0 if market.get("or_high",0) and spot>market["or_high"] else -1.0 if market.get("or_low",0) and spot<market["or_low"] else 0.0,
        "oi_pressure":oi_pressure,
        "futures":clip(float(futures.get("basis_pct",0) or 0)*25.0) if futures.get("available") else 0.0,
        "breadth":clip(breadth_score),
        "liquidity":clip(liquidity),
        "support_strength":clip((float(levels.get("support_score",0) or 0)-50)/50),
        "resistance_strength":clip((float(levels.get("resistance_score",0) or 0)-50)/50),
        "regime":str(regime.get("regime","UNKNOWN")),
        "phase":str(phase),
    }


# ---------------- PHASE 8: POSITIONING + VOLATILITY CONTEXT ----------------
def positioning_proxy(df, spot, step):
    """Transparent OI/Greek proxy; it must never be described as dealer GEX."""
    if df is None or df.empty:
        return {"available": False, "label": "UNAVAILABLE"}
    x = strike_window(df, spot, step, 8).copy()
    if x.empty:
        return {"available": False, "label": "UNAVAILABLE"}
    def weighted(side):
        oi = pd.to_numeric(x.get(f"{side}_oi", 0), errors="coerce").fillna(0).clip(lower=0)
        gamma = pd.to_numeric(x.get(f"{side}_gamma", 0), errors="coerce").fillna(0).abs()
        doi = pd.to_numeric(x.get(f"{side}_doi_5m", 0), errors="coerce").fillna(0)
        premium = pd.to_numeric(x.get(f"{side}_prem_pct", 0), errors="coerce").fillna(0)
        base = float((oi * gamma).sum())
        build = float((np.maximum(doi, 0) * (1 + np.maximum(-premium, 0) / 100.0)).sum())
        unwind = float((np.maximum(-doi, 0) * (1 + np.maximum(premium, 0) / 100.0)).sum())
        return base, build, unwind
    ce_base, ce_build, ce_unwind = weighted("ce")
    pe_base, pe_build, pe_unwind = weighted("pe")
    base_total = max(ce_base + pe_base, 1.0)
    gamma_balance = (pe_base - ce_base) / base_total
    flow_total = max(ce_build + pe_build + ce_unwind + pe_unwind, 1.0)
    flow_balance = ((pe_build - pe_unwind) - (ce_build - ce_unwind)) / flow_total
    score = max(-100.0, min(100.0, 65.0 * gamma_balance + 35.0 * flow_balance))
    label = "SUPPORT-SIDE POSITIONING" if score >= 18 else "RESISTANCE-SIDE POSITIONING" if score <= -18 else "BALANCED POSITIONING"
    return {
        "available": True, "label": label, "score": score,
        "put_share": pe_base / base_total, "call_share": ce_base / base_total,
        "pe_build": pe_build, "ce_build": ce_build, "pe_unwind": pe_unwind, "ce_unwind": ce_unwind,
    }


def realized_implied_volatility(market, structure):
    implied = _json_number(structure.get("atm_iv"))
    realized = _json_number(market.get("realized_vol_30m")) or _json_number(market.get("realized_vol_session"))
    if implied is None or realized is None or implied <= 0 or realized <= 0:
        return {"available": False, "label": "UNAVAILABLE", "implied": implied, "realized": realized}
    premium = implied - realized
    label = "IMPLIED ABOVE REALIZED" if premium > 0.03 else "REALIZED ABOVE IMPLIED" if premium < -0.03 else "IV / RV ALIGNED"
    return {"available": True, "label": label, "implied": implied, "realized": realized, "spread": premium}


def render_phase_8_context(positioning, volatility):
    st.markdown("## 🧲 Phase 8 — Positioning and volatility")
    a, b, c, d = st.columns(4)
    a.metric("Positioning proxy", positioning.get("label", "UNAVAILABLE"))
    b.metric("Proxy balance", f"{positioning.get('score', 0):+.0f}/100" if positioning.get("available") else "—")
    c.metric("IV vs realized", volatility.get("label", "UNAVAILABLE"))
    d.metric("ATM IV / realized vol", f"{volatility.get('implied', 0):.1%} / {volatility.get('realized', 0):.1%}" if volatility.get("available") else "—")
    if positioning.get("available"):
        st.caption(f"Call-side gamma/OI share {positioning['call_share']:.0%} • put-side gamma/OI share {positioning['put_share']:.0%}. The proxy combines nearby OI, available gamma, 5-minute OI flow and premium behavior.")
    if volatility.get("available"):
        st.caption(f"IV − realized volatility: {volatility['spread']:+.1%}. Realized volatility is annualized from available one-minute returns; this is context, not an options-pricing recommendation.")
    st.caption("Positioning proxy is not dealer positioning, dealer gamma exposure, or a claim about participant identity.")


def breakout_probability(side, spot, levels, market, structure, futures, breadth_score=0.0):
    """Transparent current-evidence breakout estimate; it is not an empirical hit rate."""
    upward=side=="resistance"
    precision=float(levels.get("eor_precision") or levels.get("eor") or spot) if upward else float(levels.get("eos_precision") or levels.get("eos") or spot)
    direction=1.0 if upward else -1.0
    r5=float(market.get("ret5",0) or 0); r15=float(market.get("ret15",0) or 0); vwap=float(market.get("vwap",0) or 0)
    dce=float(structure.get("ce_doi",0) or 0); dpe=float(structure.get("pe_doi",0) or 0)
    oi_confirm=((dpe-dce)/max(abs(dpe)+abs(dce),1.0)) if upward else ((dce-dpe)/max(abs(dpe)+abs(dce),1.0))
    price_confirm=direction*(r5*6+r15*2)
    vwap_confirm=direction if vwap and direction*(spot-vwap)>0 else -direction if vwap else 0.0
    futures_confirm=direction*float(futures.get("basis_pct",0) or 0)*25 if futures.get("available") else 0.0
    proximity=max(0.0,1.0-abs(spot-precision)/max(abs(spot)*0.003,1.0))
    evidence=0.30*price_confirm+0.24*oi_confirm+0.16*vwap_confirm+0.14*futures_confirm+0.10*direction*breadth_score+0.06*proximity
    probability=int(round(max(8,min(92,50+evidence*32))))
    return {"side":side,"level":precision,"probability":probability,"evidence":float(evidence),"proximity":proximity}


def render_market_state_vector(state_vector, resistance_breakout, support_breakout):
    st.markdown("## Market State")
    st.caption("Normalized live evidence vector. Breakout values are model estimates from current evidence, not historical win rates.")
    rows=[]
    for key in ("trend","momentum","volatility","vwap_relationship","opening_range","oi_pressure","futures","breadth","liquidity","support_strength","resistance_strength"):
        rows.append({"Factor":key.replace("_"," ").title(),"Score":round(float(state_vector.get(key,0)),2)})
    a,b=st.columns([3,2])
    with a:
        st.dataframe(pd.DataFrame(rows),use_container_width=True,hide_index=True)
    with b:
        st.metric(f"EOR breakout estimate · {resistance_breakout['level']:,.2f}",f"{resistance_breakout['probability']}%")
        st.metric(f"EOS breakdown estimate · {support_breakout['level']:,.2f}",f"{support_breakout['probability']}%")
        st.caption(f"Regime: {state_vector['regime']} · Phase: {state_vector['phase']}")
...
class LiveFeedCache:
    ...
    def __init__(self, token, supabase_url="", supabase_key=""):
        ...
        self.last_publish={}
        self.bridge_disabled_until=0.0
        self.rest_retry_at=0.0
        self.rest_backoff_seconds=3.0
    ...
    def _publish_tick(self, symbol, payload):
        """Publish the newest server-side tick to the browser bridge.

        The WebSocket is the primary source. Supabase is only the transport
        between the long-lived Python feed and the browser iframe.
        """
        try:
            headers={
            ...
            if r.ok:
                return True
            # A nonexistent table or endpoint cannot heal through retries. Put
            # the recorder on cooldown while the WebSocket keeps feeding charts.
            cooldown=300 if r.status_code == 404 else 30
            with self.lock:
                self.bridge_disabled_until=time.monotonic()+cooldown
                self.error=("Supabase unavailable — live chart continuing "
                            f"(HTTP {r.status_code}; retry in {cooldown}s)")
            return False
        except Exception as exc:
            with self.lock:
                self.bridge_disabled_until=time.monotonic()+30
                self.error=f"Supabase unavailable — live chart continuing ({exc})"
            return False

    def _queue_tick_for_bridge(self, key, feed):
        """Queue an optional recorder write without blocking the V3 callback."""
        ...
        sec=int(time.time())
        with self.lock:
            if time.monotonic() < self.bridge_disabled_until:
                return False
        ...
        payload={
            ...
            "previous_close":_json_number(float(vals.get("cp") or 0)),
        }
        threading.Thread(target=self._publish_tick,args=(symbol,payload),daemon=True).start()
        return True
    ...
    def _on_message(self,message):
        ...
        # The recorder is strictly asynchronous: Supabase must never block the
        # WebSocket callback or the in-memory candle source.
        for k,v in feeds.items():
            self._queue_tick_for_bridge(k,v)

    def _rest_fallback_loop(self):
        while not self.rest_stop.wait(0.5):
            try:
                with self.lock:
                    keys=list(self.keys)
                    stale=(time.time()-float(self.last_message_at or 0)) > 2.5
                    retry_at=self.rest_retry_at
                if not keys or not stale or time.monotonic() < retry_at:
                    continue
                # V3 LTP is used here; V2 LTP is deprecated.
                d=api_get3("/market-quote/ltp", self.token, {"instrument_key": ",".join(keys)})
                ...
                for key in keys:
                    ...
                    self._queue_tick_for_bridge(key,feed)
                    got += 1
                if got:
                    with self.lock:
                        self.last_message_at=time.time()
                        self.rest_backoff_seconds=3.0
                        self.rest_retry_at=time.monotonic()+3.0
            except Exception as exc:
                with self.lock:
                    # Keep retrying only as a recovery transport. Rate limits
                    # get exponential backoff, capped at one minute.
                    retry_after=0.0
                    msg=str(exc).lower()
                    if "retry-after=" in msg:
                        try:
                            retry_after=float(msg.split("retry-after=",1)[1].split(";",1)[0].split(":",1)[0])
                        except (TypeError, ValueError):
                            retry_after=0.0
                    self.rest_backoff_seconds=min(max(self.rest_backoff_seconds*2,retry_after),60.0)
                    self.rest_retry_at=time.monotonic()+self.rest_backoff_seconds
                    self.error=(f"REST LTP recovery paused {self.rest_backoff_seconds:.0f}s: {exc}")
...
def render_index_v3(name, token, include_chart=True):
    ...
    freshness = signal_freshness(name, spot, scenario, levels, market, regime, phase, cfg["step"])
    state_vector = market_state_vector(
        spot, levels, market, structure, futures, regime, phase, breadth_proxy
    )
    state_vector["signal_freshness"] = freshness
    positioning = positioning_proxy(df, spot, cfg["step"])
    volatility_context = realized_implied_volatility(market, structure)
    state_vector["positioning_proxy"] = positioning
    state_vector["volatility_context"] = volatility_context
    eor_breakout = breakout_probability("resistance", spot, levels, market, structure, futures, breadth_proxy)
    eos_breakdown = breakout_probability("support", spot, levels, market, structure, futures, breadth_proxy)
    wall_snapshots = _wall_behavior_snapshot(
        name, spot, levels, wall_rows, cfg["step"], structure, wall_migration
    )
    ...
    try:
        st.session_state["_supabase_last_status"] = save_supabase_minute(
            ...
            futures, regime, events, state_vector
        )
    ...
    try:
        wall_event_status = record_oi_wall_events(name, expiry, wall_snapshots, regime, phase)
        wall_outcome_status = settle_oi_wall_outcomes(name, cfg["step"])
    except Exception:
        wall_event_status = {"recorded": 0}
        wall_outcome_status = {"settled": 0, "pending": 0}
    if supabase_enabled():
        st.caption(f"🧱 Phase 3 wall recorder • {wall_event_status.get('recorded', 0)} states captured • {wall_outcome_status.get('settled', 0)} outcomes settled")
    # Phase 1A runs after the minute snapshot so outcomes always use the same
    # durable spot series as the prediction evidence. Database failure here is
    # isolated from the live chart and analytical dashboard.
    try:
        prediction_event_status = record_prediction_event(
            name, spot, expiry, levels, scenario, market, structure, futures,
            regime, phase, events, state_vector
        )
        prediction_outcome_status = settle_prediction_outcomes(name)
    except Exception:
        prediction_event_status = {"recorded": False}
        prediction_outcome_status = {"settled": 0, "pending": 0}
    render_outcome_recorder_status(prediction_event_status, prediction_outcome_status)
    ...
    render_market_state_vector(state_vector, eor_breakout, eos_breakdown)
    render_phase_8_context(positioning, volatility_context)
    render_oi_wall_intelligence(name, wall_snapshots)

    phase_2_setup = intraday_setup_engine(
        name, spot, levels, market, structure, futures, events, regime,
        scenario, df, cfg["step"]
    )
    phase_2_decision = render_phase_2_decision_engine(
        name, spot, levels, scenario, market, regime, phase_2_setup,
        eor_breakout, eos_breakdown, freshness
    )

    # Keep the constituent contribution read immediately below the decision
    # board; it is too important for index traders to be buried in a lower tab.
    if name in ("NIFTY 50", "BANK NIFTY"):
        breadth_context = render_contributors(token, name)
    else:
        breadth_context = {}
    phase_4_context = market_context_read(
        name, spot, market, df, structure, expiry, expected, breadth_context
    )
    render_phase_4_market_context(phase_4_context)
    option_expression = option_expression_engine(
        spot, df, cfg["step"], scenario, phase_2_setup, phase_2_decision,
        structure, expected
    )
    render_phase_5_option_expression(option_expression)
...
def validation_panel(index_name):
    st.markdown("## 🧪 Backtest / Validation")
    render_phase_1b_evidence(index_name)
    render_phase_6_model_readiness(index_name)
    st.markdown("---")
...
def render_static_live_chart(token, name, tf_minutes=5):
    ...
    try:
        ...
        render_upstox_tv_chart(candles,spot,levels,name,tf_minutes,token,live_symbol=name,live_key=cfg["key"])
        st.caption(f"🟢 LIVE — one candle series: {len(candles):,} historical candles + the current live candle. Live path: Upstox V3 WebSocket → server bridge → browser series.update(); no Streamlit rerun.")
    except Exception as exc:
...
def search_market_instruments(token, query, exchanges, segments, instrument_types="", expiry=""):
    ...
    if instrument_types:
        base_params["instrument_types"]=instrument_types
    if expiry:
        base_params["expiry"]=expiry
    try:
        # Upstox paginates the instrument-search universe. The old implementation
        # fetched only page 1, which made autocomplete look like instruments were
        # missing. Fetch a bounded set of pages and deduplicate by instrument_key.
        out=[]
        seen=set()
        for page in range(1,6):
            ...
            rows=d.get("data",[]) if isinstance(d,dict) else []
            valid=[r for r in rows if isinstance(r,dict) and r.get("instrument_key")]
            for r in valid:
                k=str(r["instrument_key"])
                if k not in seen:
...
def render_generic_market_chart(token, row, tf_minutes=5, title_prefix=""):
    ...
    render_upstox_tv_chart(
        ...
        tf_minutes, token, live_symbol=name, live_key=key
    )
    if status:
    ...
    def _market_autocomplete_fragment():
        ...
        def box(label, key, default_searchterm, placeholder, search_args, title_prefix=""):
            ...
            selected=st_searchbox(
                ...
                # Keep autocomplete responsive, but give Streamlit-searchbox enough
                # time to settle each input event instead of racing consecutive reruns.
...
def main():
    ...
    with tabs[0]: validation_panel(selected)
