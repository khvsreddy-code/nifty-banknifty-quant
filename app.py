...
def oi_heatmap(df, spot, step):
    ...
    if x.empty: return
    # Relative pressure, signed: CE positive = resistance, PE negative = support.
    ce = x.ce_strength / max(float(x.ce_strength.max()), 1)
    pe = x.pe_strength / max(float(x.pe_strength.max()), 1)
    heat = pd.DataFrame({"CE OI pressure":ce.round(2), "Strike":x.strike.astype(int), "PE OI pressure":pe.round(2)})
    st.dataframe(
        heat.style.background_gradient(subset=["CE OI pressure", "PE OI pressure"], cmap="RdYlGn_r")
        .format({"CE OI pressure":"{:.2f}","PE OI pressure":"{:.2f}"}),
...
def render_cross_index(token):
    try:
        n, ncp=underlying_ltp(token,INDEXES["NIFTY 50"]["key"])
        b, bcp=underlying_ltp(token,INDEXES["BANK NIFTY"]["key"])
    except Exception:
        return
    nm=(n/ncp-1)*100 if ncp else 0; bm=(b/bcp-1)*100 if bcp else 0
    st.markdown("## ⚔️ NIFTY vs BANK NIFTY")
    a,b,c=st.columns(3); a.metric("NIFTY",f"{n:,.2f}",f"{nm:+.2f}%"); b.metric("BANK NIFTY",f"{b:,.2f}",f"{bm:+.2f}%"); c.metric("Relative",f"{bm-nm:+.2f}%")
    if bm-nm>0.20: st.success("🟢 BANK NIFTY leadership: banking strength is exceeding Nifty move.")
    elif bm-nm<-0.20: st.error("🔴 BANK NIFTY lagging: banking weakness is dragging relative strength.")
    else: st.info("🟡 Nifty and Bank Nifty are broadly aligned.")
