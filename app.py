...
def render_upstox_tv_chart(candles, spot, levels, name):
    """TradingView-style Lightweight Charts terminal with indicators and drawings."""
    import streamlit.components.v1 as components
    ...
    for side in ("support","resistance"):
        for x in life.get(side,[])[:4]:
            ...
            seen.add(key)
            line_levels.append({"price":price,"structural":structural,"side":side,"state":x.get("state","ACTIVE")})
    payload=_json.dumps(rows); lvl=_json.dumps(line_levels); nm=_json.dumps(str(name))
    html = """<!doctype html><html><head><meta charset="utf-8">
<script src="https://unpkg.com/lightweight-charts@4.2.0/dist/lightweight-charts.standalone.production.js"></script>
<style>
html,body{margin:0;padding:0;background:#08101d;font-family:Inter,system-ui,Arial;color:#dbe7f3;overflow:hidden}
#wrap{height:720px;position:relative;border:1px solid #1d2b42;border-radius:14px;overflow:hidden;background:#08101d}
#chart{height:560px} #mini{height:135px;border-top:1px solid #17253a}
#tools{position:absolute;z-index:20;left:10px;top:9px;display:flex;gap:5px;flex-wrap:wrap;max-width:92%}
button{border:1px solid #2b3b55;background:#111c2e;color:#d9e5f2;border-radius:7px;padding:5px 8px;font-size:11px;cursor:pointer}
button:hover,button.on{background:#1b2d47;border-color:#4b6b94}.sep{width:1px;background:#2b3b55;margin:0 2px}
#status{position:absolute;right:10px;top:10px;z-index:20;font-size:10px;color:#8fa2ba;background:#0d1727dd;padding:5px 8px;border-radius:7px}
#hint{position:absolute;left:10px;bottom:143px;z-index:20;font-size:10px;color:#8295ae;pointer-events:none}
</style></head><body><div id="wrap"><div id="chart"></div><div id="mini"></div>
<div id="tools">
<button onclick="fit()">Fit</button><span class="sep"></span>
<button id="ema20" onclick="toggle('ema20')" class="on">EMA 20</button><button id="ema50" onclick="toggle('ema50')">EMA 50</button><button id="vwap" onclick="toggle('vwap')">VWAP</button><button id="bb" onclick="toggle('bb')">BB</button><button id="rsi" onclick="toggle('rsi')">RSI</button><button id="vol" onclick="toggle('vol')" class="on">Volume</button><span class="sep"></span>
<button onclick="drawMode('hline')">＋ H-Line</button><button onclick="drawMode('trend')">／ Trendline</button><button onclick="drawMode('zone')">▭ Zone</button><button onclick="undo()">Undo</button><button onclick="clearDrawings()">Clear drawings</button>
</div><div id="status">__NAME__ • Upstox • OI levels</div><div id="hint">Choose an indicator or drawing tool. Drawings are saved in this browser.</div></div>
<script>
const DATA=__DATA__; const LEVELS=__LEVELS__; const KEY='nifty_drawings_'+__NAME_JSON__;
const el=document.getElementById('chart'), mini=document.getElementById('mini');
const chart=LightweightCharts.createChart(el,{width:el.clientWidth,height:560,layout:{background:{type:'solid',color:'#08101d'},textColor:'#a9b9cb'},grid:{vertLines:{color:'#122033'},horzLines:{color:'#122033'}},crosshair:{mode:1},rightPriceScale:{borderColor:'#24354e',scaleMargins:{top:.07,bottom:.08}},timeScale:{borderColor:'#24354e',timeVisible:true,secondsVisible:false},localization:{priceFormatter:p=>Number(p).toLocaleString('en-IN',{maximumFractionDigits:2})}});
const cs=chart.addCandlestickSeries({upColor:'#10b981',downColor:'#ef4444',borderVisible:false,wickUpColor:'#10b981',wickDownColor:'#ef4444'}); cs.setData(DATA);
const vol=chart.addHistogramSeries({priceFormat:{type:'volume'},priceScaleId:'vol',color:'#49617f'}); vol.priceScale().applyOptions({scaleMargins:{top:.82,bottom:0}}); vol.setData(DATA.map(x=>({time:x.time,value:x.volume,color:x.close>=x.open?'#155e4b99':'#6b263999'})));
const overlays={};
function calcEMA(n){let a=2/(n+1),prev=null;return DATA.map(x=>{prev=prev===null?x.close:x.close*a+prev*(1-a);return {time:x.time,value:prev}})}
function addEMA(n){let k='ema'+n;if(overlays[k])return;let l=chart.addLineSeries({color:n===20?'#60a5fa':'#f59e0b',lineWidth:2,priceLineVisible:false,lastValueVisible:false});l.setData(calcEMA(n));overlays[k]=l}
function addVWAP(){if(overlays.vwap)return;let cp=0,cv=0,out=DATA.map(x=>{cp+=(x.high+x.low+x.close)/3*x.volume;cv+=x.volume;return{time:x.time,value:cv?cp/cv:x.close}});let l=chart.addLineSeries({color:'#c084fc',lineWidth:2,priceLineVisible:false,lastValueVisible:false});l.setData(out);overlays.vwap=l}
function addBB(){if(overlays.bb)return;let mid=chart.addLineSeries({color:'#94a3b8',lineWidth:1,priceLineVisible:false,lastValueVisible:false}),up=chart.addLineSeries({color:'#38bdf8',lineWidth:1,priceLineVisible:false,lastValueVisible:false}),dn=chart.addLineSeries({color:'#38bdf8',lineWidth:1,priceLineVisible:false,lastValueVisible:false});let q=[],m=[],u=[],d=[];DATA.forEach(x=>{q.push(x.close);if(q.length>20)q.shift();let av=q.reduce((a,b)=>a+b,0)/q.length,sd=Math.sqrt(q.reduce((a,b)=>a+(b-av)**2,0)/q.length);m.push({time:x.time,value:av});u.push({time:x.time,value:av+2*sd});d.push({time:x.time,value:av-2*sd})});mid.setData(m);up.setData(u);dn.setData(d);overlays.bb=[mid,up,dn]}
const rchart=LightweightCharts.createChart(mini,{width:mini.clientWidth,height:135,layout:{background:{type:'solid',color:'#08101d'},textColor:'#8194ac'},grid:{vertLines:{color:'#101c2d'},horzLines:{color:'#101c2d'}},rightPriceScale:{borderColor:'#24354e'},timeScale:{visible:false}}); const rs=rchart.addLineSeries({color:'#a78bfa',lineWidth:2,priceLineVisible:false,lastValueVisible:true});
function addRSI(){let gains=[],loss=[],out=[];for(let i=1;i<DATA.length;i++){let ch=DATA[i].close-DATA[i-1].close;gains.push(Math.max(ch,0));loss.push(Math.max(-ch,0));if(gains.length>14){gains.shift();loss.shift()}let ag=gains.reduce((a,b)=>a+b,0)/gains.length,al=loss.reduce((a,b)=>a+b,0)/loss.length;out.push({time:DATA[i].time,value:al===0?100:100-100/(1+ag/al)})}rs.setData(out);overlays.rsi=true}
function toggle(k){let b=document.getElementById(k);b.classList.toggle('on');let on=b.classList.contains('on');if(k==='ema20'){if(on)addEMA(20);else remove('ema20')}if(k==='ema50'){if(on)addEMA(50);else remove('ema50')}if(k==='vwap'){if(on)addVWAP();else remove('vwap')}if(k==='bb'){if(on)addBB();else remove('bb')}if(k==='rsi'){if(on)addRSI();else{rs.setData([]);overlays.rsi=false}}if(k==='vol')vol.applyOptions({visible:on})}
function remove(k){let x=overlays[k];if(!x)return;(Array.isArray(x)?x:[x]).forEach(s=>chart.removeSeries(s));delete overlays[k]}
LEVELS.forEach(x=>cs.createPriceLine({price:x.price,color:x.side==='support'?'#10b981':'#ef4444',lineWidth:2,lineStyle:0,axisLabelVisible:true,title:(x.side==='support'?'S ':'R ')+x.price.toFixed(2)}));
let drawings=JSON.parse(localStorage.getItem(KEY)||'[]');let svg=document.createElementNS('http://www.w3.org/2000/svg','svg');svg.style.position='absolute';svg.style.left='0';svg.style.top='0';svg.style.width='100%';svg.style.height='560px';svg.style.pointerEvents='none';document.getElementById('wrap').appendChild(svg);
function xy(d){let x=chart.timeScale().timeToCoordinate(d.t),y=cs.priceToCoordinate(d.p);return [x,y]}
function renderDrawings(){while(svg.firstChild)svg.removeChild(svg.firstChild);drawings.forEach(d=>{if(d.type==='hline'){let y=cs.priceToCoordinate(d.p);if(y==null)return;let e=document.createElementNS(svg.namespaceURI,'line');e.setAttribute('x1',0);e.setAttribute('x2',el.clientWidth);e.setAttribute('y1',y);e.setAttribute('y2',y);e.setAttribute('stroke','#38bdf8');e.setAttribute('stroke-width','2');e.setAttribute('stroke-dasharray','7 5');svg.appendChild(e)}else{let a=xy(d.a),b=xy(d.b);if(a[0]==null||b[0]==null||a[1]==null||b[1]==null)return;if(d.type==='trend'){let e=document.createElementNS(svg.namespaceURI,'line');e.setAttribute('x1',a[0]);e.setAttribute('y1',a[1]);e.setAttribute('x2',b[0]);e.setAttribute('y2',b[1]);e.setAttribute('stroke','#f59e0b');e.setAttribute('stroke-width','2');svg.appendChild(e)}else{let e=document.createElementNS(svg.namespaceURI,'rect');e.setAttribute('x',Math.min(a[0],b[0]));e.setAttribute('y',Math.min(a[1],b[1]));e.setAttribute('width',Math.abs(a[0]-b[0]));e.setAttribute('height',Math.abs(a[1]-b[1]));e.setAttribute('fill','#38bdf822');e.setAttribute('stroke','#38bdf8');e.setAttribute('stroke-dasharray','6 4');svg.appendChild(e)}}});localStorage.setItem(KEY,JSON.stringify(drawings))}
function point(ev){let r=el.getBoundingClientRect(),x=ev.clientX-r.left,y=ev.clientY-r.top;return{t:chart.timeScale().coordinateToTime(x),p:cs.coordinateToPrice(y)}}
let mode=null,pending=null;el.addEventListener('click',ev=>{if(!mode)return;let q=point(ev);if(!q.t||q.p==null)return;if(mode==='hline'){drawings.push({type:'hline',p:q.p});mode=null}else if(!pending)pending=q;else{drawings.push({type:mode,a:pending,b:q});pending=null;mode=null}renderDrawings()});
function drawMode(m){mode=m;pending=null;document.getElementById('hint').textContent=m==='hline'?'Click a price to place a horizontal line.':'Click two points to draw '+m+'.'}function undo(){drawings.pop();renderDrawings()}function clearDrawings(){drawings=[];renderDrawings()}function fit(){chart.timeScale().fitContent();rchart.timeScale().fitContent();renderDrawings()}
addEMA(20);fit();renderDrawings();new ResizeObserver(()=>{chart.applyOptions({width:el.clientWidth});rchart.applyOptions({width:mini.clientWidth});renderDrawings()}).observe(el);
</script></body></html>"""
    html=html.replace('__DATA__',payload).replace('__LEVELS__',lvl).replace('__NAME__',str(name).replace('&','&amp;').replace('<','&lt;').replace('>','&gt;')).replace('__NAME_JSON__',nm)
...
def render_index_v3(name, token):
    ...
    with h1:
        ...
        if lh.empty:
            st.caption("Persistence begins accumulating as the live chain refreshes.")
        else:
    ...
    if health.empty:
        st.caption("Level health will populate after a few refreshes.")
    else:
