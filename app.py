...
def render_upstox_tv_chart(candles, spot, levels, name, tf_minutes=5):
    ...
    payload=_json.dumps(rows); lvl=_json.dumps(line_levels); nm=_json.dumps(str(name)); tf=_json.dumps(int(tf_minutes)); sb_url, sb_key = supabase_live_config_for_browser(); sb_url_json=_json.dumps(sb_url); sb_key_json=_json.dumps(sb_key)
    html = '''<!doctype html><html><head><meta charset="utf-8">
<script src="https://unpkg.com/lightweight-charts@4.2.0/dist/lightweight-charts.standalone.production.js"></script>
<style>
*{box-sizing:border-box}html,body{margin:0;padding:0;background:#0b111b;font-family:Inter,system-ui,-apple-system,Segoe UI,Arial;color:#d8e2ee;overflow:hidden}
#wrap{height:790px;position:relative;border:1px solid #263447;border-radius:14px;overflow:hidden;background:#0b111b;box-shadow:0 14px 40px rgba(0,0,0,.28)}
#top{height:48px;display:flex;align-items:center;gap:7px;padding:7px 10px;border-bottom:1px solid #202d3e;background:#0e1724}
.symbol{font-weight:750;font-size:14px;color:#f1f5f9;padding:0 7px}.live{font-size:9px;color:#61d4a5;border:1px solid #1e624b;background:#0b2a20;border-radius:20px;padding:3px 7px}
button,select{border:1px solid #2a3a50;background:#121d2c;color:#cbd7e5;border-radius:7px;padding:6px 9px;font-size:11px;cursor:pointer;outline:none}button:hover,button.on{background:#1a2b40;border-color:#547399;color:#fff}button.danger{color:#fda4af}button.primary{background:#173554;border-color:#41698e;color:#e8f3ff}.spacer{flex:1}
#toolbar{height:42px;display:flex;align-items:center;gap:5px;padding:5px 9px;border-bottom:1px solid #202d3e;background:#0c1521;overflow-x:auto;white-space:nowrap}.group{display:flex;gap:4px;align-items:center}.label{font-size:9px;text-transform:uppercase;letter-spacing:.08em;color:#72849a;margin:0 3px}.sep{width:1px;height:23px;background:#253449;margin:0 4px}
#workspace{position:relative;height:700px}.left{position:absolute;z-index:30;left:7px;top:8px;width:34px;background:#101b2a;border:1px solid #2a3a50;border-radius:9px;display:flex;flex-direction:column;gap:3px;padding:4px}.left button{width:26px;height:26px;padding:0;font-size:14px}.left button.on{background:#214361;color:#fff}
#main{position:absolute;left:48px;right:0;top:0;height:500px}.pane{position:absolute;left:48px;right:0;top:500px;height:100px;border-top:1px solid #223044}.volpane{position:absolute;left:48px;right:0;top:600px;height:100px;border-top:1px solid #223044}.pane-title{position:absolute;left:8px;top:5px;z-index:4;font-size:9px;color:#7f91a8;background:#0b111bdd;padding:2px 5px;border-radius:4px}
#float{position:absolute;z-index:40;right:10px;top:8px;display:flex;gap:5px}.badge{font-size:9px;padding:5px 7px;border-radius:6px;background:#111c2bdf;border:1px solid #2a3a50;color:#8fa3ba}.badge strong{color:#e5edf7}.hint{position:absolute;z-index:40;left:55px;bottom:7px;font-size:9px;color:#71849a;pointer-events:none}
.modal{position:absolute;z-index:60;right:9px;top:52px;width:300px;background:#101b2a;border:1px solid #30435c;border-radius:10px;box-shadow:0 18px 50px rgba(0,0,0,.5);padding:10px;display:none}.modal h4{margin:0 0 8px;font-size:12px}.grid{display:grid;grid-template-columns:1fr 1fr;gap:5px}.small{font-size:9px;color:#8193a8;margin:7px 0 4px}
...
<script>
const DATA=__DATA__, LEVELS=__LEVELS__, TF_MINUTES=__TF_MINUTES__, KEY='nifty_tv_drawings_'+__NAME_JSON__;
...
const chart=LightweightCharts.createChart(mainEl,{width:mainEl.clientWidth,height:500,layout:{background:{type:'solid',color:'#0b111b'},textColor:'#9fb0c4'},grid:{vertLines:{color:'#172334'},horzLines:{color:'#172334'}},crosshair:{mode:1},rightPriceScale:{borderColor:'#2a3a50',scaleMargins:{top:.07,bottom:.08}},timeScale:{borderColor:'#2a3a50',timeVisible:true,secondsVisible:false},localization:{priceFormatter:p=>Number(p).toLocaleString('en-IN',{maximumFractionDigits:2})}});
const cs=chart.addCandlestickSeries({upColor:'#22c55e',downColor:'#ef4444',borderUpColor:'#22c55e',borderDownColor:'#ef4444',wickUpColor:'#22c55e',wickDownColor:'#ef4444'});cs.setData(DATA);const ls=chart.addLineSeries({color:'#d8e2ee',lineWidth:2,visible:false,priceLineVisible:false,lastValueVisible:false});ls.setData(DATA.map(x=>({time:x.time,value:x.close})));
const volChart=LightweightCharts.createChart(volEl,{width:volEl.clientWidth,height:100,layout:{background:{type:'solid',color:'#0b111b'},textColor:'#71849a'},grid:{vertLines:{color:'#111d2b'},horzLines:{color:'#111d2b'}},rightPriceScale:{borderColor:'#26374d'},timeScale:{visible:false}});const vol=volChart.addHistogramSeries({priceFormat:{type:'volume'},priceScaleId:'vol'});vol.priceScale().applyOptions({scaleMargins:{top:.15,bottom:.05}});vol.setData(DATA.map(x=>({time:x.time,value:x.volume,color:x.close>=x.open?'#1d7f5c99':'#9b3b4699'})));
const pane=LightweightCharts.createChart(paneEl,{width:paneEl.clientWidth,height:100,layout:{background:{type:'solid',color:'#0b111b'},textColor:'#71849a'},grid:{vertLines:{color:'#111d2b'},horzLines:{color:'#111d2b'}},rightPriceScale:{borderColor:'#26374d'},timeScale:{visible:false}});let p1=pane.addLineSeries({color:'#60a5fa',lineWidth:2,priceLineVisible:false,lastValueVisible:true}),p2=pane.addLineSeries({color:'#f59e0b',lineWidth:1,priceLineVisible:false,lastValueVisible:true});
...
function addLine(key,data,color,width=2){if(overlays[key])return;let s=chart.addLineSeries({color,lineWidth:width,priceLineVisible:false,lastValueVisible:false});s.setData(data);overlays[key]=s}function remove(k){let s=overlays[k];if(!s)return;(Array.isArray(s)?s:[s]).forEach(x=>chart.removeSeries(x));delete overlays[k]}
...
// Browser-side live bridge: chart stays mounted while live ticks arrive.
const SB_URL=__SB_URL__, SB_KEY=__SB_KEY__, LIVE_SYMBOL=__NAME_JSON__;
let liveBar=null;
let lastLivePrice=0;
function applyLiveTick(t){
  const price=Number(t.price||0), ts=Math.floor(new Date(t.ts).getTime()/1000);
  if(!price || !ts) return;
  lastLivePrice=price;
  ...
  if(!liveBar || liveBar.time!==bucket){
    const base=(last && last.time===bucket)
      ? last
      : {time:bucket,open:price,high:price,low:price,close:price,volume:0};
    liveBar={...base,close:price,high:Math.max(Number(base.high||price),price),low:Math.min(Number(base.low||price),price)};
...
async function startLiveBridge(){
  ...
  try{
    ...
    const client=mod.createClient(SB_URL,SB_KEY);
    client.channel('oi-pulse-live-'+LIVE_SYMBOL)
      .on('postgres_changes',{event:'INSERT',schema:'public',table:'live_ticks',filter:'symbol=eq.'+LIVE_SYMBOL},p=>applyLiveTick(p.new))
      .subscribe();
  }catch(e){}
}
async function pollLatestTick(){
  if(!SB_URL||!SB_KEY) return;
  try{
    const url=SB_URL+'/rest/v1/live_ticks?symbol=eq.'+encodeURIComponent(LIVE_SYMBOL)+'&select=ts,price,previous_close&order=ts.desc&limit=1';
    const r=await fetch(url,{headers:{apikey:SB_KEY,Authorization:'Bearer '+SB_KEY}});
    if(!r.ok) return;
    const a=await r.json();
    if(a&&a.length) applyLiveTick(a[0]);
  }catch(e){}
}
window.lastLiveAt=0;
...
setInterval(()=>{
  ...
  if(window.lastLiveAt && Date.now()-window.lastLiveAt>3000){
    badge.textContent='● STALE — waiting for live tick';
...
addLine('ema20',ema(20),'#60a5fa');setPane('RSI');fit();renderDrawings();new ResizeObserver(()=>{chart.applyOptions({width:mainEl.clientWidth});pane.applyOptions({width:paneEl.clientWidth});volChart.applyOptions({width:volEl.clientWidth});renderDrawings()}).observe(mainEl);chart.timeScale().subscribeVisibleTimeRangeChange(()=>{renderDrawings();try{let r=chart.timeScale().getVisibleRange();if(r){volChart.timeScale().setVisibleRange(r);pane.timeScale().setVisibleRange(r)}}catch(e){}});
    ...
    html=html.replace('__DATA__',payload).replace('__LEVELS__',lvl).replace('__TF_MINUTES__',tf).replace('__NAME__',str(name).replace('&','&amp;').replace('<','&lt;').replace('>','&gt;')).replace('__NAME_JSON__',nm).replace('__SB_URL__',sb_url_json).replace('__SB_KEY__',sb_key_json)
    components.html(html,height=805,scrolling=False)
...
def live_underlying_ltp(token, key):
    """Persistent V3 WebSocket first; REST quote is the fallback."""
...
class LiveFeedCache:
    ...
    def _publish_tick(self, key, feed):
        ...
        if not self.supabase_url or not self.supabase_key:
            return
        symbol=self.symbol_by_key.get(str(key))
        if not symbol:
            return
        try:
            vals=ws_extract(feed or {})
...
def render_index_v3(name, token, include_chart=True):
    ...
    if include_chart:
        ...
        try:
            ...
            st.caption("🟢 LIVE — price ticks update the mounted chart independently of the analytics pulse.")
            st.caption(
...
def render_static_live_chart(token, name, tf_minutes=5):
    ...
    try:
        ...
        render_upstox_tv_chart(candles,spot,levels,name,tf_minutes)
        st.caption("🟢 LIVE — price ticks arrive independently from Streamlit analytics. The chart is mounted once and is not rebuilt on each analytics pulse.")
    except Exception as exc:
...
def main():
    ...
    with st.sidebar:
        ...
        refresh=st.slider("Analytics pulse (seconds)",3,15,5)
        ...
        if st.button("Archive today's 1m data to Supabase", use_container_width=True):
            with st.spinner("Downloading today's complete 1-minute index history…"):
                ar=archive_today_prices(token, ["NIFTY 50","BANK NIFTY"])
            if ar.get("saved"):
                st.success(f"Archived {ar.get('rows',0):,} 1-minute rows.")
            else:
                st.warning(ar.get("reason","No rows archived."))
        st.caption("The chart uses the persistent V3 feed when available; the minute recorder continues capturing OI/levels for research.")
