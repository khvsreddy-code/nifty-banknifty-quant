...
def render_upstox_tv_chart(candles, spot, levels, name):
    ...
    payload=_json.dumps(rows); lvl=_json.dumps(line_levels); nm=_json.dumps(str(name)); sb_url, sb_key = supabase_live_config_for_browser(); sb_url_json=_json.dumps(sb_url); sb_key_json=_json.dumps(sb_key)
...
<script>
const DATA=__DATA__, LEVELS=__LEVELS__, KEY='nifty_tv_drawings_'+__NAME_JSON__;
...
function applyLiveTick(t){
  ...
  if(!liveBar || liveBar.time!==minute){
    const base=(last && last.time===minute)?last:{time:minute,open:price,high:price,low:price,close:price,volume:0};
    liveBar={...base,close:price,high:Math.max(base.high,price),low:Math.min(base.low,price)};
    if(!DATA.length || DATA[DATA.length-1].time<minute) DATA.push(liveBar);
  }else{
    liveBar.close=price; liveBar.high=Math.max(liveBar.high,price); liveBar.low=Math.min(liveBar.low,price);
  }
  try{cs.update(liveBar);ls.update({time:minute,value:price});}catch(e){}
  const badge=document.querySelector('.live'); if(badge) badge.textContent='● LIVE '+price.toLocaleString('en-IN',{maximumFractionDigits:2});
}
...
addLine('ema20',ema(20),'#60a5fa');setPane('RSI');fit();renderDrawings();new ResizeObserver(()=>{chart.applyOptions({width:mainEl.clientWidth});pane.applyOptions({width:paneEl.clientWidth});volChart.applyOptions({width:volEl.clientWidth});renderDrawings()}).observe(mainEl);chart.timeScale().subscribeVisibleTimeRangeChange(()=>{renderDrawings();try{let r=chart.timeScale().getVisibleRange();if(r){volChart.timeScale().setVisibleRange(r);pane.timeScale().setVisibleRange(r)}}catch(e){}});
    ...
    html=html.replace('__DATA__',payload).replace('__LEVELS__',lvl).replace('__NAME__',str(name).replace('&','&amp;').replace('<','&lt;').replace('>','&gt;')).replace('__NAME_JSON__',nm).replace('__SB_URL__',sb_url_json).replace('__SB_KEY__',sb_key_json)
