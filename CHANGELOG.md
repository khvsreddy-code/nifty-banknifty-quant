# V11.8 — Visible live-feed state

- Added an always-visible browser-side feed state: LIVE / RECONNECTING / FEED ERROR.
- Shows last tick time in IST and live tick age in seconds.
- Shows the actual Upstox HTTP error (including HTTP 401) instead of silently freezing.
- A stale feed transitions to RECONNECTING after 5 seconds without a tick.
- HTTP 401 / invalid-token errors are shown as FEED ERROR.
- Supabase failures remain non-fatal because direct Upstox V3 polling is the authoritative fallback.

# V11.7 — TRUE LIVE CANDLE FIX

- One Lightweight Charts candle series keeps historical candles and the current live candle together.
- Supabase is treated as a transport, not as the live-data source of record. A stale Supabase row no longer blocks the live path.
- Browser-side Upstox V3 LTP fallback now runs at ~1 second whenever the Supabase row is not newer than the last received tick.
- No Streamlit timer/rerun is used for live chart movement.
- Market autocomplete now fetches bounded paginated search results (up to 5 pages) instead of only page 1.
- Existing TradingCoreOS UI, indicators, drawings, IST time axis and bar countdown are retained.

## V11.5 — Unified historical + live chart fix

- One Lightweight Charts candle series now stitches multi-month Upstox V3 history with the current live candle.
- Added a browser-side 1-second Upstox V3 LTP fallback so live updates do not depend on Streamlit reruns or Supabase Realtime.
- Kept Supabase Realtime as an optional live transport when configured.
- Added IST tick labels and an in-chart bar countdown.
- Optimized EMA/SMA live updates incrementally so large historical datasets do not freeze the browser.
- Added stale-tick protection so closed-market quotes cannot overwrite the latest historical candle.

## V11.1.1 — Market Autocomplete

- Added live autocomplete suggestions to Stocks, Futures, Options and MCX search.
- Suggestions query the Upstox Instrument Search API as the user types.
- Search input is debounced to 180 ms.
- Autocomplete updates are isolated to a Streamlit fragment; there is no timer-based dashboard refresh.
- The currently selected live chart remains mounted while the user edits the search term.
- Added `streamlit-searchbox==0.1.24`.
...
## V11.1 — true no-rerun live transport + multi-asset explorer
- Removed the 5-second Streamlit fragment.
- Fixed the V11 websocket/Supabase bridge so the process-lifetime feed receives
  the Supabase URL/key and actually publishes live ticks.
- Chart bootstrap now starts the websocket itself, so live charting does not
  depend on analytics rendering.
- Added searchable Stocks, Futures, Options and MCX Futures using Upstox
  Instrument Search.
- Generic instruments use instrument_key as the realtime symbol.
- Updated the browser chart to use Lightweight Charts 5.2 `autoSize`, Magnet
  crosshair, realtime `update()`, price-line/scale improvements and a cleaner
  white TradingView-style presentation.
